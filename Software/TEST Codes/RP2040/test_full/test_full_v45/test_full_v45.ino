...
void readIRSensors() {
  // The current IR harness is rotated 180 degrees relative to the logical
  // FL/FR/RL/RR naming used by the event logic, so read the opposite physical
  // pin into each logical sensor slot.
  gIrFL = digitalRead(PIN_IR_RR);
  gIrFR = digitalRead(PIN_IR_RL);
  gIrRL = digitalRead(PIN_IR_FR);
  gIrRR = digitalRead(PIN_IR_FL);
