...
struct RuntimeSettings {
  ...
  int junctionStopDistance_x100 = 1200;
  int waitBeforeTurn_x100 = 500;
  int irStableTime_x100 = 10;
  int deadEndDistance_x100 = 1800;
  uint8_t mode = 1;
};
...
int settingsIndex = 0;      // 0=PID, 1=Motor Tune, 2=Junction Stop, 3=Save
int pidIndex = 0;           // 0=P, 1=I, 2=D, 3=b, 4=PID Test, 5=Back
...
constexpr int PID_COUNT = 6;
constexpr int IR_SENSOR_COUNT = 5;
...
const float SIDE_OPEN_THRESHOLD = 18.0f;
...
constexpr int EVENT_TEST_CONFIG_COUNT = 8;
...
bool ultrasonicSnapshotChanged(const SensorSnapshot& a, const SensorSnapshot& b);
...
void updatePIDPreview() {
  ...
  pidPreviewInterpretedError = corridorInterpretedErrorForHeading(pidTestHeading);
...
SensorSnapshot captureSensorSnapshot() {
  ...
  snapshot.distN = distN;
  snapshot.distE = distE;
  snapshot.distS = distS;
  snapshot.distW = distW;
...
bool ultrasonicSnapshotChanged(const SensorSnapshot& a, const SensorSnapshot& b) {
  ...
  if (a.ultrasonicValid != b.ultrasonicValid) return true;
  if (!a.ultrasonicValid || !b.ultrasonicValid) return false;

  return fabs(a.distN - b.distN) > epsilon ||
         fabs(a.distE - b.distE) > epsilon ||
         fabs(a.distS - b.distS) > epsilon ||
         fabs(a.distW - b.distW) > epsilon;
}

bool sensorSnapshotChanged(const SensorSnapshot& a, const SensorSnapshot& b) {
  if (a.irMask != b.irMask) return true;
  return ultrasonicSnapshotChanged(a, b);
}
...
EventName classifyStableSensorEvent(const SensorSnapshot& snapshot, Heading heading, bool allowFinishEvent) {
  if (allowFinishEvent && (snapshot.irMask & 0x0F) != 0) {
  ...
  bool leftOpenStable = left >= SIDE_OPEN_THRESHOLD;
  bool rightOpenStable = right >= SIDE_OPEN_THRESHOLD;

  if (leftOpenStable && rightOpenStable) return EventName::TJunction;
  if (leftOpenStable) return EventName::LeftTurn;
  if (rightOpenStable) return EventName::RightTurn;
  if (front <= gDeadEndDistanceCm) return EventName::DeadEnd;
...
float frontDistance() { return distanceForHeading(currentHeading); }
float leftDistance() { return distanceForHeading(turnLeftOf(currentHeading)); }
float rightDistance() { return distanceForHeading(turnRightOf(currentHeading)); }
float backDistance() { return distanceForHeading(oppositeOf(currentHeading)); }

bool leftOpen() {
  return ultrasonicFresh() && leftDistance() >= SIDE_OPEN_THRESHOLD;
}

bool rightOpen() {
  return ultrasonicFresh() && rightDistance() >= SIDE_OPEN_THRESHOLD;
}
...
void updateMazeTestLogic() {
  ...
  SensorSnapshot stableEventSnapshot;
  bool hasStableEvent = consumeStableSensorSnapshot(mazeTestHandledStableVersion, stableEventSnapshot);
  EventName stableDetectedEvent = hasStableEvent
    ? classifyStableSensorEvent(stableEventSnapshot, mazeTestHeading, true)
    : EventName::Idle;

  auto leftDistanceForMaze = [&]() -> float {
    return distanceForHeadingInSnapshot(stableEventSnapshot, turnLeftOf(mazeTestHeading));
  };
  auto rightDistanceForMaze = [&]() -> float {
    return distanceForHeadingInSnapshot(stableEventSnapshot, turnRightOf(mazeTestHeading));
  };
  ...
  switch (mazeTestPhase) {
    ...
    case MazeTestPhase::StartSeek:
      mazeTestEvent = EventName::Start;
      if (hasStableEvent &&
          (stableEventSnapshot.irMask & 0x0C) == 0x0C &&
          stableEventSnapshot.ultrasonicValid &&
          leftDistanceForMaze() <= gJunctionStopDistance &&
          rightDistanceForMaze() <= gJunctionStopDistance) {
    ...
    case MazeTestPhase::ApproachWall: {
      mazeTestEvent = EventName::ApproachWall;
      if (!hasStableEvent || !stableEventSnapshot.ultrasonicValid) return;

      float front = distanceForHeadingInSnapshot(stableEventSnapshot, mazeTestHeading);
      bool leftIsOpen = leftDistanceForMaze() >= SIDE_OPEN_THRESHOLD;
      bool rightIsOpen = rightDistanceForMaze() >= SIDE_OPEN_THRESHOLD;

      if (front > gJunctionStopDistance) return;

      Heading leftHeading = turnLeftOf(mazeTestHeading);
      Heading rightHeading = turnRightOf(mazeTestHeading);
      mazeTestPendingHeading = mazeTestHeading;

      switch (gMode) {
        case 2:
          if (rightIsOpen) mazeTestPendingHeading = rightHeading;
          else if (leftIsOpen) mazeTestPendingHeading = leftHeading;
          break;
        case 3:
          if (leftIsOpen) mazeTestPendingHeading = leftHeading;
          else if (rightIsOpen) mazeTestPendingHeading = rightHeading;
          break;
        case 4:
          if (leftIsOpen && rightIsOpen) {
            mazeTestPendingHeading = (leftDistanceForMaze() >= rightDistanceForMaze()) ? leftHeading : rightHeading;
          } else if (leftIsOpen) {
            mazeTestPendingHeading = leftHeading;
          } else if (rightIsOpen) {
            mazeTestPendingHeading = rightHeading;
          }
          break;
        default:
          if (rightIsOpen) mazeTestPendingHeading = rightHeading;
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (settingsIndex) {
      ...
      case 1:
        ...
        break;
      case 2:
        openDigitEditor("Jct Stop", &workingSettings.junctionStopDistance_x100, 0, 9999);
        break;
      case 3:
        commitGlobalsForRuntime();
        currentScreen = Screen::Root;
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (eventTestConfigIndex) {
      ...
      case 3:
        ...
        break;
      case 4:
        openDigitEditor("MoveTm", &cfg.moveTime_x100, 0, 9999);
        break;
      case 5:
...
void drawSettingsScreen() {
  ...
  drawMenuItem(44, settingsIndex == 2, "Jct Stop: " + formatX100(workingSettings.junctionStopDistance_x100));
...
void drawPIDTestScreen() {
  ...
  String line1 = String("Dir:") + headingToString(pidTestHeading) + " b:" + formatX100(workingSettings.pidCurveB_x100);
  String line2 = String("L:") + formatDistanceCm(averagedDistanceForHeading(turnLeftOf(pidTestHeading))) +
                 " R:" + formatDistanceCm(averagedDistanceForHeading(turnRightOf(pidTestHeading)));
  String line3 = String("Raw:") + formatSignedFloat(pidPreviewRawError, 2);
  String line4 = String("Int:") + formatSignedFloat(pidPreviewInterpretedError, 2);
  String line5 = String("PID:") + formatSignedFloat(pidPreviewCorrection, 2);
  String line6 = "y=s*0.6*(|x|/0.6)^b";
...
void drawEventTestConfigScreen() {
  ...
  items[2] = "Side: " + formatX100(cfg.sideThreshold_x100);
  items[3] = "Act: " + formatX100(cfg.actionTime_x100);
  items[4] = "Move: " + formatX100(cfg.moveTime_x100);
  items[5] = "Turn: " + String(turnChoiceToString(cfg.turnChoice));
  items[6] = "Run";
  items[7] = "Back";
...
void drawUltrasonicSensorTestScreen() {
  drawHeader("US Test");
  u8g2.setFont(u8g2_font_5x8_tf);
  String line1 = String("Raw N:") + formatDistanceCm(distN) + " E:" + formatDistanceCm(distE);
  String line2 = String("Raw S:") + formatDistanceCm(distS) + " W:" + formatDistanceCm(distW);
  String line3 = String("Avg N:") + formatDistanceCm(avgDistN) + " E:" + formatDistanceCm(avgDistE);
  String line4 = String("Avg S:") + formatDistanceCm(avgDistS) + " W:" + formatDistanceCm(avgDistW);
  String line5 = String("NS:") + formatSignedFloat(interpretedNS, 2) + " EW:" + formatSignedFloat(interpretedEW, 2);
  u8g2.drawStr(0, 21, line1.c_str());
  u8g2.drawStr(0, 28, line2.c_str());
  u8g2.drawStr(0, 35, line3.c_str());
  u8g2.drawStr(0, 42, line4.c_str());
  u8g2.drawStr(0, 49, line5.c_str());
  u8g2.setFont(u8g2_font_5x8_tf);
  u8g2.drawStr(0, 54, ultrasonicFresh() ? "Data: fresh" : "Data: waiting");
...
void followCorridorAtSpeed(float forwardSpeed) {
  float left = averagedDistanceForHeading(turnLeftOf(currentHeading));
  float right = averagedDistanceForHeading(turnRightOf(currentHeading));
  bool leftWallPresent = left < SIDE_OPEN_THRESHOLD;
  bool rightWallPresent = right < SIDE_OPEN_THRESHOLD;

  if (leftWallPresent && rightWallPresent) {
    float error = corridorInterpretedErrorForHeading(currentHeading);
...
void runSelectedEventTest() {
  ...
  Kd = gD;
  SensorSnapshot stableEventSnapshot;
  bool hasStableEvent = consumeStableSensorSnapshot(eventTestHandledStableVersion, stableEventSnapshot);
  bool leftIsOpen = leftOpen();
  bool rightIsOpen = rightOpen();
  ...
  auto resolveRandomEvent = [&]() -> EventName {
    return classifyStableSensorEvent(stableEventSnapshot, currentHeading, true);
  };
  ...
  switch (eventTestPhase) {
    ...
    case EventTestPhase::StartSeek:
      ...
      if (hasStableEvent &&
          (stableEventSnapshot.irMask & 0x0C) == 0x0C &&
          stableEventSnapshot.ultrasonicValid &&
          distanceForHeadingInSnapshot(stableEventSnapshot, turnLeftOf(currentHeading)) <= sideThreshold &&
          distanceForHeadingInSnapshot(stableEventSnapshot, turnRightOf(currentHeading)) <= sideThreshold) {
        eventTestPhase = EventTestPhase::Detect;
        eventTestStartedMs = millis();
        previousError = 0.0f;
        integral = 0.0f;
    ...
    case EventTestPhase::Detect: {
      if (selectedEventTest == EventTestCase::Corridor) {
      ...
      if (selectedEventTest == EventTestCase::Finish) {
        currentEventName = EventName::Corridor;
        if (hasStableEvent && classifyStableSensorEvent(stableEventSnapshot, currentHeading, true) == EventName::Finish) {
      ...
      EventName stableDetectedEvent = classifyStableSensorEvent(stableEventSnapshot, currentHeading,
        selectedEventTest == EventTestCase::Finish || selectedEventTest == EventTestCase::Random);

      if (selectedEventTest != EventTestCase::Random &&
          selectedEventTest != EventTestCase::DeadEnd &&
          selectedEventTest != EventTestCase::TJunctionStraight &&
          stableDetectedEvent != eventTestCaseToEventName(selectedEventTest)) {
        currentEventName = EventName::Corridor;
        followCorridor();
        return;
      }
    ...
    case EventTestPhase::WaitBeforeTurn:
      ...
      if ((millis() - eventTestStartedMs) >= (uint32_t)(gWaitBeforeTurnSeconds * 1000.0f)) {
        ...
        eventTestPhase = EventTestPhase::MoveNewHeading;
        eventTestStartedMs = millis();
      }
      stopAll();
      return;

    case EventTestPhase::MoveNewHeading:
      currentEventName = (eventTestResolvedEvent == EventName::TJunction)
        ...
        : eventTestResolvedEvent;
      if ((millis() - eventTestStartedMs) >= currentEventTestMoveMs()) {
        eventTestPhase = EventTestPhase::Detect;
        eventTestStartedMs = millis();
        previousError = 0.0f;
        integral = 0.0f;
    ...
    case EventTestPhase::StraightToSideThreshold:
      currentEventName = EventName::TJunctionStraight;
      if (sideUltrasonicWithin(sideThreshold)) {
        eventTestPhase = EventTestPhase::Detect;
        eventTestStartedMs = millis();
        previousError = 0.0f;
        integral = 0.0f;
...
void updateExecutionLogic(bool& stopNow, EventName& displayedEvent) {
  ...
  Kd = gD;
  SensorSnapshot stableEventSnapshot;
  bool hasStableEvent = consumeStableSensorSnapshot(mainHandledStableVersion, stableEventSnapshot);
  EventName stableDetectedEvent = hasStableEvent
    ? classifyStableSensorEvent(stableEventSnapshot, currentHeading, true)
    : EventName::Idle;
  ...
  switch (executionPhase) {
    case ExecutionPhase::Corridor:
      if (hasStableEvent && stableDetectedEvent == EventName::Finish) {
      ...
      if (hasStableEvent && stableDetectedEvent == EventName::DeadEnd) {
    ...
    case ExecutionPhase::ApproachWall:
      displayedEvent = leftIsOpen && rightIsOpen ? EventName::TJunction
                     : leftIsOpen ? EventName::LeftTurn
                     : rightIsOpen ? EventName::RightTurn
                     : EventName::ApproachWall;

      if (front <= gJunctionStopDistance) {
        pendingHeading = chooseNextHeading(leftIsOpen, rightIsOpen);
        ...
        phaseStartedMs = millis();
        stopAll();
        displayedEvent = eventForTurnHeading(pendingHeading);
        return;
      }
...
void setup() {
  ...
  activeSettings.junctionStopDistance_x100 = 1200;
  activeSettings.waitBeforeTurn_x100 = 500;
  activeSettings.irStableTime_x100 = 10;
  activeSettings.deadEndDistance_x100 = 1800;
  activeSettings.mode = 1;

  for (int i = 0; i < EVENT_TEST_COUNT; ++i) {
    eventTestConfigs[i].heading = 0;
    eventTestConfigs[i].stopThreshold_x100 = 1000;
    eventTestConfigs[i].sideThreshold_x100 = 1800;
