...
void handlePidInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gPidIndex) {
    ...
    case 12:
      ...
      resetImuPid();
      break;
    case 13: resetImuPid(); gCurrentScreen = Screen::IMUPIDTest; break;
...
void drawIMUPIDTestScreen() {
  ...
  u8g2.drawStr(0, 36, line);
  snprintf(line, sizeof(line), "Cmd:%+1.3f S:%+d", gImu.lastCompensationCmd, (activeSettings.imuSign_x100 < 0) ? -1 : 1);
  u8g2.drawStr(0, 45, line);
  snprintf(line, sizeof(line), "Bias:%+2.2f Hold:%c", gImu.gyroBiasZDps, (millis() < gImu.compensationHoldUntilMs) ? 'Y' : 'N');
  u8g2.drawStr(0, 54, line);
