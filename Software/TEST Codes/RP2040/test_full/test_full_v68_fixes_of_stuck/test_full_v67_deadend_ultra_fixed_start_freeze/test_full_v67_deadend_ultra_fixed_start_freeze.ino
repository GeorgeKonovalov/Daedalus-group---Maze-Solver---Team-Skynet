  ...
  switch (gExecutive.state) {
    ...
    case NavState::FrontNormalize: {
...

      {
        ...
        bool finishNow = false;
        EventType correctionCandidate =
          correctionStageCandidate(frame, gExecutive.correctionConfirmEnabled);
        if (gExecutive.correctionConfirmEnabled &&
            updateEventConfirmState(gExecutive.eventConfirm, correctionCandidate, confirmed)) {
          (void)applyCompensationEventOverride(frame,
                                              confirmed,
                                              gExecutive.heading,
                                              gExecutive.latchedEvent,
                                              gExecutive.pendingHeading,
                                              gExecutive.finishArmed,
                                              gExecutive.tJunctionRaised,
                                              gExecutive.deadEndRaised,
                                              false,
                                              TurnChoice::Left,
                                              leaveNormalize,
          ...
          gExecutive.displayEvent = gExecutive.latchedEvent;
          if (finishNow) {
            enterPhase(NavState::Finished, EventType::Finish);
            return;
          }
          if (leaveNormalize) {
            ...
            prepareOverallSpeedAfterStopEvent(gExecutive.latchedEvent);
      ...
      bool correctionEventReady =
        !gExecutive.correctionConfirmEnabled || gExecutive.latchedEvent != EventType::Corridor;
      if (!frontNormalizationComplete(frame, gExecutive.latchedEvent) || !correctionEventReady) {
        gExecutive.displayEvent = gExecutive.latchedEvent;
        return;
      }
