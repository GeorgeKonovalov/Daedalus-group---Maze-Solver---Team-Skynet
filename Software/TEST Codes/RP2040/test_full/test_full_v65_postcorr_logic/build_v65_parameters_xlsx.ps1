...
$outFile = Join-Path $root 'test_full_v65_postcorr_logic_parameters.xlsx'
...
$rows = @(
  ...
  [PSCustomObject]@{
    ...
    Notes = 'v65 keeps the strict front < FrontStop rule. Runtime now corrects first, then determines left/right/T-junction from the corrected pose before changing heading.'
  }
  [PSCustomObject]@{
  ...
  [PSCustomObject]@{
    ...
    Notes = 'Current default preserved in v65.'
  }
  [PSCustomObject]@{
    Parameter = 'IMU I'
    Value = '1.7000'
  ...
  [PSCustomObject]@{
    ...
    Notes = 'Renewed default preserved in v65.'
  }
  [PSCustomObject]@{
  ...
  [PSCustomObject]@{
    ...
    Value = 'Runtime flag'
    Description = 'Legacy internal release guard retained in the codebase from the previous version.'
    Units = 'flag'
    Menu = 'Internal only'
    Code = 'holdPostCorrectionRelease(), AcquireCorridor in preview/runtime/event-test flows'
    Notes = 'Retained in v65, but the main runtime navigation fix now comes from post-correction event determination rather than relying on this guard.'
  }
  [PSCustomObject]@{
