...
constexpr uint8_t ULTRASONIC_AVG_COUNT = 3;
...
volatile int16_t gEncoderRawDelta = 0;
...
float latestDistanceForHeading(Heading heading);
...
float latestDistanceForHeading(Heading heading) {
  float ultrasonicByIndex[4] = {gDistN, gDistE, gDistS, gDistW};
  return ultrasonicByIndex[headingToIndex(heading)];
}
...
bool corridorSignatureForFrame(const PerceptionFrame& frame) {
  return frame.ultrasonicValid &&
         frame.frontRaw > gCfg.deadEndDistanceCm &&
         frame.ir.frontLeft &&
         frame.ir.frontRight;
}
...
EventType classifyCandidateEvent(const PerceptionFrame& frame) {
  ...
  float frontCm = frame.frontRaw;
  float leftCm = frame.leftRaw;
  float rightCm = frame.rightRaw;
...
PerceptionFrame buildPerceptionFrame(Heading heading) {
  ...
  frame.northRaw = gDistN;
  frame.eastRaw = gDistE;
  frame.southRaw = gDistS;
  frame.westRaw = gDistW;
  ...
  frame.frontRaw = latestDistanceForHeading(heading);
  frame.leftRaw = latestDistanceForHeading(turnLeftOf(heading));
  frame.rightRaw = latestDistanceForHeading(turnRightOf(heading));
  frame.backRaw = latestDistanceForHeading(oppositeOf(heading));
...
bool updateEventConfirmation(const PerceptionFrame& frame, EventType& confirmedEvent) {
  ...
  // This stays in the main loop on purpose: UART ultrasonics arrive asynchronously,
  // so counting only fresh packets after the configured gap is safer than running
  // a timer ISR that would just re-check stale data.
  ...
  if (!state.active || state.candidate != candidate) {
    ...
    state.lastSampleId = frame.ultrasonicSampleId;
    return false;
  }

  if (frame.ultrasonicSampleId == 0 || frame.ultrasonicSampleId == state.lastSampleId) {
    return false;
  }
...
static bool updateEventConfirmState(EventConfirmState& state, EventType candidate, EventType& confirmedEvent) {
  ...
  uint32_t now = millis();
  // The event-test path mirrors the runtime logic: wait the configured gap, then
  // only count a new match when a fresh ultrasonic packet has actually arrived.
  if (!state.active || state.candidate != candidate) {
    state.active = true;
    state.candidate = candidate;
    state.matches = 1;
    state.nextCheckMs = now + secondsToMs(gCfg.sensingIntervalS);
    state.lastSampleId = gUltrasonicSampleId;
    return false;
  }

  if (gUltrasonicSampleId == 0 || gUltrasonicSampleId == state.lastSampleId) {
    return false;
  }
  if (now < state.nextCheckMs) {
    return false;
...
void handleSettingsInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gSettingsIndex) {
    ...
    case 10: openDigitEditor("Match N", "x", &workingSettings.eventConfirmCount, 1, 9, "How many identical", "events confirm state", Screen::Settings, 1, 0); break;
...
void drawSettingsScreen() {
  ...
  snprintf(items[10], sizeof(items[10]), "Match N:%d", activeSettings.eventConfirmCount);
...
void drawUI() {
  ...
  if (!forceDraw && (now - gLastUiDrawMs) < UI_REFRESH_INTERVAL_MS) {
    return;
  }
