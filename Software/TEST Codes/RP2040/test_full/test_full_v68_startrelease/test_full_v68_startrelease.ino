...
constexpr uint8_t OLED_ADDR = 0x3C; //i2c address of the oled display
U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, U8X8_PIN_NONE); // OLED structure
...
constexpr int32_t ENCODER_EDGES_PER_MENU_STEP = 2;
...
constexpr float IMU_OUTPUT_SLEW_PER_S = 3.0f;          // Limit yaw-hold output steps so the chassis does not snap.
...
constexpr float IMU_BODY_AXIS_C45 = 0.70710678f;       // IMU board is mounted at 45 deg to the robot body axes.

// ======================================================
// Settings storage (fixed-point menu values)
// ======================================================
struct RuntimeSettings {
  ...
  int imuP_x10000 = 39000;
  int imuI_x10000 = 17000;
  int imuD_x10000 = 0;
...
struct RuntimeConfig {
  bool wallPidEnabled = true;
  bool pid1WallSymmetric = true;
  float pidDeadband1WCm = 0.6f;
  float p = 0.25f;
  float i = 0.0f;
  float d = 0.02f;
  float pid1WallP = 0.35f;
  float pid1WallI = 0.0f;
  float pid1WallD = 0.04f;
  float pid1WallRightP = 0.35f;
  float pid1WallRightI = 0.0f;
  float pid1WallRightD = 0.04f;
  ...
  float overallStart = 0.20f;
  float overallRampK = 2.0f;
  bool leftTurnRampEnabled = true;
  bool rightTurnRampEnabled = true;
  bool tJunctionRampEnabled = true;
  bool deadEndRampEnabled = true;
  ...
  float frontStopDistanceCm = 15.0f;
  float frontHoldReferenceCm = 3.5f;
  float frontPidP = 0.15f;
  float frontPidI = 0.0f;
  float frontPidD = 0.04f;
  float turnYawToleranceDeg = 3.0f;
...
enum class PidTestMode : uint8_t {
  TwoWall,
  OneWallLeft,
  OneWallRight,
  NoWall
...
struct ImuLateralRuntime {
  float filteredLateralBodyG = 0.0f;
  float previousError = 0.0f;
  float integral = 0.0f;
  uint32_t previousMs = 0;
};
...
struct AttemptRecord {
  bool valid = false;
  bool hadDeadEnd = false;
  uint32_t durationMs = 0;
  RouteMode chosenMode = RouteMode::Selection;
};
...
struct PlannerState {
  ...
  RouteMode currentAttemptRecordMode = RouteMode::Right;
  RouteMode currentAttemptExecutionMode = RouteMode::Right;
  bool currentAttemptHadDeadEnd = false;
  AttemptRecord rightAttempt;
  AttemptRecord leftAttempt;
  AttemptRecord selectionAttempt;
};
...
struct ExecutiveState {
  ...
  // True only when the post-stop compensation phase is allowed to re-confirm
  // and override the pending event. Legacy dead-end stops keep this false so
  // they only align and then reverse direction.
  bool correctionConfirmEnabled = true;
  bool postCorrectionReleaseActive = false;
...
PidRuntime gPid1WallRight;
PidRuntime gFrontPid;
WallReferenceState gWallRef;
ImuPidRuntime gImuPid;
ImuLateralRuntime gImuLatPid;
ImuState gImu;
PidSource gActivePidSource = PidSource::None;
bool gImuPidTestArmed = false;
float gOverallSpeedActual = 1.0f;
uint32_t gOverallSpeedUpdateMs = 0;
...
PidTestMode gPidTestMode = PidTestMode::TwoWall;
...
float currentImuLateralCompensation(float forwardSpeed);
...
static bool planNextAttempt(RouteMode& executionMode, RouteMode& recordMode, const char*& errorText);
static const char* routeModeShortString(RouteMode mode);
...
static bool applyCompensationEventOverride(const PerceptionFrame& frame,
                                           ...
                                           EventType& latchedEvent,
                                           Heading& pendingHeading,
                                           bool& finishArmed,
                                           bool& tJunctionRaised,
                                           bool& deadEndRaised,
                                           bool preferConfiguredTJunction,
                                           TurnChoice configuredTurn,
                                           bool& leaveNormalize,
                                           bool& finishNow);
static bool legacyDeadEndStopTriggered(const PerceptionFrame& frame);
static bool correctionStageUltrasonicDeadEndDetected(const PerceptionFrame& frame);
static bool postCorrectionDeadEndTriggered(const PerceptionFrame& frame, EventType currentEvent);
static EventType correctionStageCandidate(const PerceptionFrame& frame, bool allowDeadEndConfirm);
static EventType postCorrectionTurnCandidate(const PerceptionFrame& frame);
...
void resetPidRuntime(PidRuntime& runtime);
float computePid(float error, PidSource source);
float computePidWithRuntime(PidRuntime& runtime, float error, PidSource source,
                            float pGain, float iGain, float dGain);
float computeWallError(const PerceptionFrame& frame, PidSource source);
float computeWallCorrection(const PerceptionFrame& frame, PidSource source);
...
void followSmartForced(Heading heading, float forwardSpeed, PidSource forcedSource);
static void setOverallSpeedActual(float speed);
static void syncOverallSpeedActualToTarget();
static void initializeRuntimeOverallSpeed(bool startLimited);
static void updateOverallSpeedActualForState(NavState state);
static bool holdPostCorrectionRelease(bool& releaseActive, const PerceptionFrame& frame);
static bool frontNormalizationComplete(const PerceptionFrame& frame, EventType eventType);
static void runFrontNormalization(Heading heading, const PerceptionFrame& frame, EventType eventType);
...
void handlePidTestInput(int encoderDelta, ButtonEvent buttonEvent);
...
const char* pidTestModeToString(PidTestMode mode);
void updatePidTestDrive();
...
void formatShortDuration(char* buffer, size_t size, uint32_t durationMs);
void formatPlannerAttemptLine(char* buffer, size_t size, const char* label, const AttemptRecord& record);
...
void drawRunScreen();
void drawUI();
void startPreviewExecution();
void updatePreviewExecution();

// ======================================================
// Implementation sections
// ======================================================
// Helpers
constexpr float PID_WALL_ERROR_DEADBAND_CM = 0.6f;
constexpr float PID_1WALL_FULL_ERROR_CM = 8.0f;
constexpr float FRONT_HOLD_FULL_ERROR_CM = 4.0f;
constexpr float FRONT_HOLD_ACCEPTANCE_CM = 0.3f;

float applyErrorDeadband(float error, float deadband) {
...
/*
  One-wall sign convention
  ------------------------
  Positive lateral correction always means "move to the robot's right".

  Therefore:
  - Left wall  too close  -> positive error  -> shift right
  - Left wall  too far    -> negative error  -> shift left
  - Right wall too close  -> negative error  -> shift left
  - Right wall too far    -> positive error  -> shift right

  This keeps the left-wall and right-wall controllers explicitly mirrored.
*/
float computeOneWallScaledError(float distanceCm, bool leftWall) {
  float targetCm = gCfg.pidWallDistanceCm;
  float errorCm = 0.0f;
  if (leftWall) {
    errorCm = targetCm - distanceCm;
  } else {
    errorCm = distanceCm - targetCm;
  }
  return clampFloat(errorCm / PID_1WALL_FULL_ERROR_CM, -1.0f, 1.0f);
}
...
void commitRuntimeConfig() {
  ...
  gCfg.wallPidEnabled = (activeSettings.wallPidEnabled != 0);
  gCfg.pid1WallSymmetric = (activeSettings.pid1WallSymmetryEnabled != 0);
  ...
  gCfg.pid1WallP = fixedToFloat3(activeSettings.pid1WallP_x1000);
  gCfg.pid1WallI = fixedToFloat3(activeSettings.pid1WallI_x1000);
  gCfg.pid1WallD = fixedToFloat3(activeSettings.pid1WallD_x1000);
  gCfg.pid1WallRightP = fixedToFloat3(activeSettings.pid1WallRightP_x1000);
  gCfg.pid1WallRightI = fixedToFloat3(activeSettings.pid1WallRightI_x1000);
  gCfg.pid1WallRightD = fixedToFloat3(activeSettings.pid1WallRightD_x1000);
  ...
  gCfg.imuLatP = fixedToFloat4(activeSettings.imuLatP_x10000);
  gCfg.imuLatI = fixedToFloat4(activeSettings.imuLatI_x10000);
  gCfg.imuLatD = fixedToFloat4(activeSettings.imuLatD_x10000);
  gCfg.imuLatFilterAlpha = clampFloat(fixedToFloat2(activeSettings.imuLatFilterAlpha_x100), 0.01f, 1.0f);
  ...
  gCfg.overallStart = clampFloat(fixedToFloat2(activeSettings.overallStart_x100), 0.0f, 1.0f);
  gCfg.overallRampK = clampFloat(fixedToFloat2(activeSettings.overallRampK_x100), 0.0f, 20.0f);
  gCfg.leftTurnRampEnabled = (activeSettings.leftTurnRampEnabled != 0);
  gCfg.rightTurnRampEnabled = (activeSettings.rightTurnRampEnabled != 0);
  gCfg.tJunctionRampEnabled = (activeSettings.tJunctionRampEnabled != 0);
  gCfg.deadEndRampEnabled = (activeSettings.deadEndRampEnabled != 0);
  ...
  gCfg.servoStopFL = clampFloat(fixedToFloat3(activeSettings.servoStopFL_x1000), 0.0f, 180.0f);
  gCfg.servoStopFR = clampFloat(fixedToFloat3(activeSettings.servoStopFR_x1000), 0.0f, 180.0f);
  gCfg.servoStopRR = clampFloat(fixedToFloat3(activeSettings.servoStopRR_x1000), 0.0f, 180.0f);
  gCfg.servoStopRL = clampFloat(fixedToFloat3(activeSettings.servoStopRL_x1000), 0.0f, 180.0f);
  ...
  gCfg.frontHoldReferenceCm = fixedToFloat2(activeSettings.frontHoldReference_x100);
  gCfg.frontPidP = fixedToFloat3(activeSettings.frontPidP_x1000);
  gCfg.frontPidI = fixedToFloat3(activeSettings.frontPidI_x1000);
  gCfg.frontPidD = fixedToFloat3(activeSettings.frontPidD_x1000);
  gCfg.turnYawToleranceDeg = fixedToFloat2(activeSettings.turnYawTolerance_x100);
  ...
  if (!gExecutive.running && !gEventTestRun.active && !gPreviewRun.active) {
    gOverallSpeedActual = gCfg.overallSpeedScale;
    gOverallSpeedUpdateMs = millis();
  }
}
...
void resetPidRuntime(PidRuntime& runtime) {
  ...
  runtime.derivativeFiltered = 0.0f;
  runtime.previousMs = millis();
}

void resetPid() {
  resetPidRuntime(gPid);
}

void resetAllWallPidRuntimes() {
  resetPidRuntime(gPid);
  resetPidRuntime(gPid1WallSym);
  resetPidRuntime(gPid1WallLeft);
  resetPidRuntime(gPid1WallRight);
}
...
void resetImuLateralPid() {
  gImuLatPid.filteredLateralBodyG = 0.0f;
  gImuLatPid.previousError = 0.0f;
  gImuLatPid.integral = 0.0f;
  gImuLatPid.previousMs = millis();
}

void resetFrontPid() {
  resetPidRuntime(gFrontPid);
}
...
const char* navStateToString(NavState state) {
  switch (state) {
    ...
    case NavState::FrontNormalize:  return "FrontHold";
...
static void clearAttemptRecord(AttemptRecord& record) {
  ...
  record.durationMs = 0;
  record.chosenMode = RouteMode::Selection;
}

static void resetPlannerAttemptHistory() {
  clearAttemptRecord(gPlanner.rightAttempt);
  clearAttemptRecord(gPlanner.leftAttempt);
  clearAttemptRecord(gPlanner.selectionAttempt);
  gPlanner.lastAttemptDurationMs = 0;
  gPlanner.currentAttemptRecordMode = RouteMode::Right;
  gPlanner.currentAttemptExecutionMode = RouteMode::Right;
  gPlanner.currentAttemptHadDeadEnd = false;
}

static const char* routeModeShortString(RouteMode mode) {
  switch (mode) {
    case RouteMode::Right:     return "R";
    case RouteMode::Left:      return "L";
    case RouteMode::Selection: return "S";
    default:                   return "-";
  }
}
...
static bool planNextAttempt(RouteMode& executionMode, RouteMode& recordMode, const char*& errorText) {
  errorText = nullptr;
  RouteMode configuredMode = currentRouteMode();
  if (configuredMode != RouteMode::Sequence) {
    executionMode = configuredMode;
    recordMode = configuredMode;
    return true;
  }

  switch (gPlanner.sequenceSlot % 3) {
    case 0:
      executionMode = RouteMode::Right;
      recordMode = RouteMode::Right;
      return true;
    case 1:
      executionMode = RouteMode::Left;
      recordMode = RouteMode::Left;
      return true;
    default:
      recordMode = RouteMode::Selection;
      if (!gPlanner.rightAttempt.valid || !gPlanner.leftAttempt.valid) {
        executionMode = RouteMode::Selection;
        errorText = "Need R+L first";
        return false;
      }
      if (gPlanner.rightAttempt.hadDeadEnd && gPlanner.leftAttempt.hadDeadEnd) {
        executionMode = RouteMode::Selection;
        errorText = "Err: both dead-end";
        return false;
      }
      if (gPlanner.rightAttempt.hadDeadEnd) {
        executionMode = RouteMode::Left;
        return true;
      }
      if (gPlanner.leftAttempt.hadDeadEnd) {
        executionMode = RouteMode::Right;
        return true;
      }
      executionMode = (gPlanner.rightAttempt.durationMs <= gPlanner.leftAttempt.durationMs)
                        ? RouteMode::Right
                        : RouteMode::Left;
      return true;
  }
...
const char* nextTrialModeToString() {
  RouteMode executionMode = RouteMode::Selection;
  RouteMode recordMode = RouteMode::Selection;
  const char* errorText = nullptr;
  if (!planNextAttempt(executionMode, recordMode, errorText)) {
    return "Sel ERR";
  }
  if (recordMode == RouteMode::Selection && currentRouteMode() == RouteMode::Sequence) {
    return (executionMode == RouteMode::Right) ? "Sel->Right" : "Sel->Left";
  }
  return routeModeToString(executionMode);
}
...
PidSource pidSourceForTestMode(PidTestMode mode) {
  switch (mode) {
    case PidTestMode::TwoWall:      return PidSource::TwoWall;
    case PidTestMode::OneWallLeft:  return PidSource::LeftWall;
    case PidTestMode::OneWallRight: return PidSource::RightWall;
    case PidTestMode::NoWall:
    default:                        return PidSource::None;
  }
}

const char* pidTestModeToString(PidTestMode mode) {
  switch (mode) {
    case PidTestMode::TwoWall:      return "2Wall";
    case PidTestMode::OneWallLeft:  return "1Wall L";
    case PidTestMode::OneWallRight: return "1Wall R";
    case PidTestMode::NoWall:       return "NoWall";
    default:                        return "-";
  }
}
...
void formatPlannerAttemptLine(char* buffer, size_t size, const char* label, const AttemptRecord& record) {
  ...
  char duration[12];
  formatShortDuration(duration, sizeof(duration), record.durationMs);
  if (record.chosenMode == RouteMode::Selection) {
    snprintf(buffer, size, "%s:%s %s", label, duration, record.hadDeadEnd ? "DE" : "OK");
  } else {
    snprintf(buffer, size, "%s:%s %s %s", label, duration,
             routeModeShortString(record.chosenMode),
             record.hadDeadEnd ? "DE" : "OK");
  }
}
...
void onEncoderEdge() {
  ...
  // Use only channel A interrupts and sample channel B for direction.
  // This keeps the menu sensitivity at two edges per UI step while reducing
  // ISR load and making the encoder less likely to miss a detent under timing load.
  if (a == b) {
    gEncoderRawDelta++;
  } else {
    gEncoderRawDelta--;
...
uint8_t buildIrMaskFromPins() {
  uint8_t mask = 0;
  // Restore the direct front/rear interpretation of the IR pin names while
  // keeping left/right unchanged:
  //   FL -> NW, FR -> NE, RR -> SE, RL -> SW
  if (isIrTriggered(gIrFL)) mask |= 0x04; // NW / FL
  if (isIrTriggered(gIrFR)) mask |= 0x08; // NE / FR
  if (isIrTriggered(gIrRL)) mask |= 0x01; // SW / RL
  if (isIrTriggered(gIrRR)) mask |= 0x02; // SE / RR
  return mask;
}
...
void readIRSensors() {
  ...
  //   RL <- physical RR
  //   RR <- physical RL
  gIrFL = digitalRead(PIN_IR_RL); //RL
  gIrFR = digitalRead(PIN_IR_RR); //RR
  gIrRL = digitalRead(PIN_IR_FL); //FL
  gIrRR = digitalRead(PIN_IR_FR); //FR
...
bool corridorSignatureForFrame(const PerceptionFrame& frame) {
  return frame.ultrasonicValid &&
         frame.left <= gCfg.corridorWallThresholdCm &&
         frame.right <= gCfg.corridorWallThresholdCm;
}
...
EventType classifyCandidateEvent(const PerceptionFrame& frame, bool finishArmed, bool tJunctionStraightArmed) {
  ...
  bool leftWall = leftCm <= gCfg.corridorWallThresholdCm;
  ...
  if (frontCm < gCfg.deadEndDistanceCm &&
      frame.ir.frontLeft &&
      frame.ir.frontRight) {
    return EventType::DeadEnd;
  }

  if (frame.ir.frontLeft &&
      frame.ir.frontRight &&
      leftWall &&
      rightWall) {
  ...
  if (!leftWall && rightWall && frontClose) {
    return EventType::LeftTurn;
  }

  if (!rightWall && leftWall && frontClose) {
    return EventType::RightTurn;
  }
...
static bool legacyDeadEndStopTriggered(const PerceptionFrame& frame) {
  // Legacy dead-end handling is still driven by the original front IR +
  // front-distance signature. During that case we only align and reverse;
  // the correction-stage override logic is intentionally disabled.
  return frame.ultrasonicValid &&
...
static bool correctionStageUltrasonicDeadEndDetected(const PerceptionFrame& frame) {
  // This stricter all-ultrasonic dead-end rule is only used while the robot
  // is already stopped and correcting its position after FrontStop.
  return frame.ultrasonicValid &&
         frame.front < gCfg.corridorWallThresholdCm &&
         frame.left < gCfg.corridorWallThresholdCm &&
         frame.right < gCfg.corridorWallThresholdCm;
}

static bool postCorrectionDeadEndTriggered(const PerceptionFrame& frame, EventType currentEvent) {
  // Once the robot is already stopped for some other event, dead-end must no
  // longer be allowed to hijack the stop through the old front-IR signature.
  // After correction completes we only accept a dead-end override if the
  // forward, left, and right ultrasonic channels are all blocked.
  if (currentEvent == EventType::DeadEnd || currentEvent == EventType::Start) {
    return false;
  }

  return correctionStageUltrasonicDeadEndDetected(frame);
}

static EventType correctionStageCandidate(const PerceptionFrame& frame, bool allowDeadEndConfirm) {
  // During the active correction phase we only keep confirming left/right/
  // T-junction/finish. Dead-end is intentionally deferred until correction is
  // complete so a close stop for some other event cannot be hijacked by the
  // old front-IR based dead-end rule.
  (void)allowDeadEndConfirm;

  switch (frame.candidate) {
...
float wallErrorInterpretLimit(PidSource source) {
  return (source == PidSource::None) ? PID_INTERPRET_LIMIT_CM : 1.0f;
}
...
float computeWallError(const PerceptionFrame& frame, PidSource source) {
  switch (source) {
    ...
    case PidSource::LeftWall:
      return computeOneWallScaledError(frame.left, true);
    case PidSource::RightWall:
      return computeOneWallScaledError(frame.right, false);
    case PidSource::None:
    default:
      return 0.0f;
  }
...
float computePidWithRuntime(PidRuntime& runtime, float error, PidSource source,
  ...
  float derivativeRaw = (shapedError - runtime.previousError) / dt;
  float dAlpha = gCfg.pidDFilterAlpha;
  runtime.derivativeFiltered += dAlpha * (derivativeRaw - runtime.derivativeFiltered);

  runtime.integral += shapedError * dt;
  runtime.integral = clampFloat(runtime.integral, -PID_INTEGRAL_LIMIT, PID_INTEGRAL_LIMIT);

  float output = pGain * shapedError +
                 iGain * runtime.integral +
                 dGain * runtime.derivativeFiltered;

  runtime.previousError = shapedError;
...
float computePid(float error, PidSource source) {
  return computePidWithRuntime(gPid, error, source, gCfg.p, gCfg.i, gCfg.d);
}

float computeWallCorrection(const PerceptionFrame& frame, PidSource source) {
  if (!wallPidUsageEnabled() || source == PidSource::None) {
    resetAllWallPidRuntimes();
    return 0.0f;
  }

  float wallError = computeWallError(frame, source);
  if (source == PidSource::TwoWall) {
    wallError = applyErrorDeadband(wallError, gCfg.pidDeadband2W);
    return computePid(wallError, source);
  }

  wallError = applyErrorDeadband(wallError, gCfg.pidDeadband1WCm / PID_1WALL_FULL_ERROR_CM);
  if (gCfg.pid1WallSymmetric) {
    return computePidWithRuntime(gPid1WallSym, wallError, source,
                                 gCfg.pid1WallP, gCfg.pid1WallI, gCfg.pid1WallD);
  }

  if (source == PidSource::LeftWall) {
    return computePidWithRuntime(gPid1WallLeft, wallError, source,
                                 gCfg.pid1WallP, gCfg.pid1WallI, gCfg.pid1WallD);
  }

  return computePidWithRuntime(gPid1WallRight, wallError, source,
                               gCfg.pid1WallRightP, gCfg.pid1WallRightI, gCfg.pid1WallRightD);
}
...
float currentImuLateralCompensation(float forwardSpeed) {
  if (!imuUsageEnabled() || fabsf(forwardSpeed) <= 0.0001f) {
    resetImuLateralPid();
    return 0.0f;
  }

  uint32_t now = millis();
  float dt = (gImuLatPid.previousMs == 0) ? 0.01f : (now - gImuLatPid.previousMs) / 1000.0f;
  dt = clampFloat(dt, 0.01f, 0.20f);
  gImuLatPid.previousMs = now;

  // Rotate the IMU planar axes into robot-body forward/lateral axes, then
  // damp only the signed lateral component while NoWalls is active.
  float aForwardBody = IMU_BODY_AXIS_C45 * gImu.ax + IMU_BODY_AXIS_C45 * gImu.ay;
  float aLateralBody = -IMU_BODY_AXIS_C45 * gImu.ax + IMU_BODY_AXIS_C45 * gImu.ay;
  (void)aForwardBody;

  gImuLatPid.filteredLateralBodyG +=
    gCfg.imuLatFilterAlpha * (aLateralBody - gImuLatPid.filteredLateralBodyG);

  float error = -gImuLatPid.filteredLateralBodyG;
  float derivative = (error - gImuLatPid.previousError) / dt;

  gImuLatPid.integral += error * dt;
  gImuLatPid.integral = clampFloat(gImuLatPid.integral,
                                   -IMU_PID_INTEGRAL_LIMIT,
                                   IMU_PID_INTEGRAL_LIMIT);

  float output = gCfg.imuLatP * error +
                 gCfg.imuLatI * gImuLatPid.integral +
                 gCfg.imuLatD * derivative;

  gImuLatPid.previousError = error;
  return clampFloat(output, -1.0f, 1.0f);
}

static void setOverallSpeedActual(float speed) {
  gOverallSpeedActual = clampFloat(speed, 0.0f, gCfg.overallSpeedScale);
  gOverallSpeedUpdateMs = millis();
}

static void syncOverallSpeedActualToTarget() {
  setOverallSpeedActual(gCfg.overallSpeedScale);
}

static bool rampResumeEnabledForEvent(EventType eventType) {
  switch (eventType) {
    case EventType::LeftTurn:  return gCfg.leftTurnRampEnabled;
    case EventType::RightTurn: return gCfg.rightTurnRampEnabled;
    case EventType::TJunction: return gCfg.tJunctionRampEnabled;
    case EventType::DeadEnd:   return gCfg.deadEndRampEnabled;
    default:                   return false;
  }
}

static void prepareOverallSpeedAfterStopEvent(EventType eventType) {
  if (rampResumeEnabledForEvent(eventType)) {
    setOverallSpeedActual(0.0f);
  } else {
    syncOverallSpeedActualToTarget();
  }
}

static void initializeRuntimeOverallSpeed(bool startLimited) {
  if (startLimited) {
    setOverallSpeedActual(min(gCfg.overallStart, gCfg.overallSpeedScale));
  } else {
    syncOverallSpeedActualToTarget();
  }
}
...
static void updateOverallSpeedActualForState(NavState state) {
  uint32_t now = millis();
  float dt = (gOverallSpeedUpdateMs == 0) ? 0.0f : (now - gOverallSpeedUpdateMs) / 1000.0f;
  gOverallSpeedUpdateMs = now;

  if (state == NavState::StartSeek) {
    gOverallSpeedActual = min(gCfg.overallStart, gCfg.overallSpeedScale);
    return;
  }

  if (state == NavState::FrontNormalize) {
    gOverallSpeedActual = 0.0f;
    return;
  }

  if (gOverallSpeedActual > gCfg.overallSpeedScale) {
    gOverallSpeedActual = gCfg.overallSpeedScale;
    return;
  }

  if (gOverallSpeedActual < gCfg.overallSpeedScale && dt > 0.0f) {
    gOverallSpeedActual = min(gOverallSpeedActual + gCfg.overallRampK * dt,
                              gCfg.overallSpeedScale);
  }
}
...
static bool holdPostCorrectionRelease(bool& releaseActive, const PerceptionFrame& frame) {
  ...
  // After a correction-stage event has been confirmed we must let the robot
  // actually depart in the newly selected direction instead of immediately
  // feeding the fresh heading back into the same FrontStop gate.
  if (frame.ultrasonicValid && frame.front >= gCfg.frontStopDistanceCm) {
    releaseActive = false;
    return false;
  }
...
static float computeFrontNormalizedError(float measuredFrontCm) {
  float errorCm = measuredFrontCm - gCfg.frontHoldReferenceCm;
  return clampFloat(errorCm / FRONT_HOLD_FULL_ERROR_CM, -1.0f, 1.0f);
}

static float computeFrontPidCorrection(float measuredFrontCm) {
  uint32_t now = millis();
  float dt = (gFrontPid.previousMs == 0) ? 0.01f : (now - gFrontPid.previousMs) / 1000.0f;
  dt = clampFloat(dt, 0.01f, 0.20f);
  gFrontPid.previousMs = now;

  float error = computeFrontNormalizedError(measuredFrontCm);
  float deadband = FRONT_HOLD_ACCEPTANCE_CM / FRONT_HOLD_FULL_ERROR_CM;
  error = applyErrorDeadband(error, deadband);

  float derivativeRaw = (error - gFrontPid.previousError) / dt;
  gFrontPid.derivativeFiltered +=
    gCfg.pidDFilterAlpha * (derivativeRaw - gFrontPid.derivativeFiltered);

  gFrontPid.integral += error * dt;
  gFrontPid.integral = clampFloat(gFrontPid.integral, -PID_INTEGRAL_LIMIT, PID_INTEGRAL_LIMIT);

  float output = gCfg.frontPidP * error +
                 gCfg.frontPidI * gFrontPid.integral +
                 gCfg.frontPidD * gFrontPid.derivativeFiltered;

  gFrontPid.previousError = error;
  return clampFloat(output, -1.0f, 1.0f);
}

static bool yawNormalizationComplete() {
  if (!imuUsageEnabled() || !gImu.referenceValid) {
    return true;
  }

  float allowedYawErrorDeg = max(gCfg.turnYawToleranceDeg, gImu.combinedDeadbandDeg);
  return fabsf(gImu.yawErrorDeg) <= allowedYawErrorDeg;
}

static bool deadEndNormalizationComplete(const PerceptionFrame& frame) {
  if (!frame.ultrasonicValid) return false;

  PidSource controlSource = stabilizedPidSource(frame, gActivePidSource);
  if (wallPidUsageEnabled() && controlSource != PidSource::None) {
    float wallError = computeWallError(frame, controlSource);
    float allowedWallError = (controlSource == PidSource::TwoWall)
                               ? max(gCfg.pidDeadband2W, 0.01f)
                               : max(gCfg.pidDeadband1WCm / PID_1WALL_FULL_ERROR_CM, 0.01f);
    if (fabsf(wallError) > allowedWallError) {
      return false;
    }
  }

  return yawNormalizationComplete();
}

static bool eventUsesFrontHold(EventType eventType) {
  switch (eventType) {
    case EventType::DeadEnd:
...
static bool frontNormalizationComplete(const PerceptionFrame& frame, EventType eventType) {
  if (!eventUsesFrontHold(eventType)) {
    return deadEndNormalizationComplete(frame);
  }

  if (!frame.ultrasonicValid) return false;
  if (fabsf(frame.front - gCfg.frontHoldReferenceCm) > FRONT_HOLD_ACCEPTANCE_CM) return false;
  return yawNormalizationComplete();
}
...
static void runFrontNormalization(Heading heading, const PerceptionFrame& frame, EventType eventType) {
  ...
  if (controlSource != PidSource::None) {
    resetImuLateralPid();
    lateralCorrection = computeWallCorrection(frame, controlSource);
  } else {
    resetImuLateralPid();
  }

  float forwardCorrection = !eventUsesFrontHold(eventType)
                              ? 0.0f
                              : computeFrontPidCorrection(frame.front);
...
static void driveBodyFrameLimited(float baseVx, float baseVy,
  ...
  // overallSpeedScale remains the configured target; overallSpeedActual is the
  // live ramped execution speed used by the drive path.
  basePlus *= gOverallSpeedActual;
  baseMinus *= gOverallSpeedActual;
...
void driveRobotFrame(float lateralCmd, float forwardCmd, float rotationCmd, Heading heading) {
  ...
  // Positive lateralCmd always means "shift to the robot's right" regardless
  // of maze heading. The heading mapping below rotates that signed request into
  // the body-frame X/Y axes used by the omni drive mixer.
...
void driveRobotFrameAdvanced(float lateralCmd, float baseForwardCmd, float forwardCompCmd, float rotationCmd, Heading heading) {
  ...
  switch (heading) {
    case Heading::North:
      baseVy = baseForwardCmd;
      compVx = lateralCmd;
      compVy = forwardCompCmd;
      break;
    case Heading::East:
      baseVx = baseForwardCmd;
      compVy = -lateralCmd;
      compVx = forwardCompCmd;
      break;
    case Heading::South:
      baseVy = -baseForwardCmd;
      compVx = -lateralCmd;
...
/*
  ...
  3. followSmart() computes:
       lateral correction  <- wall PID, or IMU lateral drift suppression in NoWalls
...
void followSmart(Heading heading, float forwardSpeed) {
  ...
  if (controlSource == PidSource::None) {
    correction = currentImuLateralCompensation(forwardSpeed);
  } else {
    resetImuLateralPid();
    correction = computeWallCorrection(frame, controlSource);
  }
  float rotationComp = currentImuRotationCompensation(gImuPid);
...
void followSmartForced(Heading heading, float forwardSpeed, PidSource forcedSource) {
  ...
  float correction = 0.0f;
  if (forcedSource == PidSource::None) {
    correction = currentImuLateralCompensation(forwardSpeed);
  } else {
    resetImuLateralPid();
    correction = computeWallCorrection(frame, forcedSource);
  }
  float rotationComp = currentImuRotationCompensation(gImuPid);
  driveRobotFrame(correction, forwardSpeed, rotationComp, heading);
}
...
void beginExecution() {
  ...
  gExecutive.correctionConfirmEnabled = true;
  gExecutive.postCorrectionReleaseActive = false;
  ...
  initializeRuntimeOverallSpeed(true);
...
static void finishExecutionAt(uint32_t stopMs) {
  ...
  record.valid = true;
  record.hadDeadEnd = gPlanner.currentAttemptHadDeadEnd;
  record.durationMs = durationMs;
  record.chosenMode = gPlanner.currentAttemptExecutionMode;
...
static bool eventTriggersEnabledForState(NavState state) {
  // Navigation events are only allowed to trigger while we are freely driving
  // the corridor. Once an event has been confirmed and we are handling it
...
static void startPreviewPhase(NavState state, EventType displayEvent) {
  ...
  gPreviewRun.phaseStartedMs = millis();
  if (state == NavState::FrontNormalize) {
    setOverallSpeedActual(0.0f);
  } else {
    gOverallSpeedUpdateMs = millis();
  ...
  resetFrontPid();
  gPreviewRun.eventConfirm.active = false;
  gPreviewRun.eventConfirm.candidate = EventType::Idle;
  gPreviewRun.eventConfirm.matches = 0;
  gPreviewRun.eventConfirm.nextCheckMs = 0;
  gPreviewRun.eventConfirm.lastSampleId = 0;
  gPreviewRun.corridorConfirm.active = false;
  gPreviewRun.corridorConfirm.matches = 0;
  gPreviewRun.corridorConfirm.nextCheckMs = 0;
  gPreviewRun.corridorConfirm.lastSampleId = 0;
}

void startPreviewExecution() {
  gPreviewRun.active = true;
  ...
  gPreviewRun.correctionConfirmEnabled = true;
  gPreviewRun.postCorrectionReleaseActive = false;
  ...
  gPreviewRun.phaseStartedMs = gPreviewRun.startedMs;
  initializeRuntimeOverallSpeed(true);
  startPreviewPhase(NavState::StartSeek, EventType::Start);
}
...
void updatePreviewExecution() {
  ...
  const char* errorText = nullptr;
  if (!planNextAttempt(executionMode, recordMode, errorText)) {
    gPreviewRun.active = false;
    syncOverallSpeedActualToTarget();
    stopAll();
    return;
  }

  if (!gPreviewRun.active) {
  ...
  updateOverallSpeedActualForState(gPreviewRun.state);
  ...
  switch (gPreviewRun.state) {
    case NavState::StartSeek: {
      ...
      PerceptionFrame startFrame = frame;
      startFrame.corridorSignature = startSignatureForFrame(frame);
      if (updateCorridorConfirmState(gPreviewRun.corridorConfirm, startFrame.corridorSignature)) {
        gPreviewRun.latchedEvent = EventType::Start;
        gPreviewRun.pendingHeading = gPreviewRun.heading;
        gPreviewRun.correctionConfirmEnabled = false;
        gPreviewRun.postCorrectionReleaseActive = false;
        startPreviewPhase(NavState::FrontNormalize, EventType::Start);
      }
      return;
    }
    ...
    case NavState::Corridor: {
      if (frame.candidate == EventType::SensorWait) {
        gPreviewRun.displayEvent = EventType::SensorWait;
        return;
      }

      EventType confirmed = EventType::Idle;
      if (eventTriggersEnabledForState(gPreviewRun.state) &&
        ...
        if (confirmed == EventType::DeadEnd) {
          ...
          gPreviewRun.correctionConfirmEnabled = false;
          gPreviewRun.postCorrectionReleaseActive = false;
          startPreviewPhase(NavState::FrontNormalize, EventType::DeadEnd);
          return;
        }
        if (confirmed == EventType::TJunctionStraight) {
          gPreviewRun.displayEvent = EventType::TJunctionStraight;
          return;
        }
        if (confirmed == EventType::LeftTurn ||
          ...
          gPreviewRun.correctionConfirmEnabled = true;
          gPreviewRun.postCorrectionReleaseActive = false;
          startPreviewPhase(NavState::ApproachWall, confirmed);
          return;
        }
      }

      if (frame.ultrasonicValid && frame.front < gCfg.frontStopDistanceCm) {
        bool legacyDeadEnd = legacyDeadEndStopTriggered(frame);
        gPreviewRun.correctionConfirmEnabled = !legacyDeadEnd;
        gPreviewRun.postCorrectionReleaseActive = false;
        gPreviewRun.latchedEvent = legacyDeadEnd ? EventType::DeadEnd : EventType::Corridor;
        gPreviewRun.pendingHeading = legacyDeadEnd ? oppositeOf(gPreviewRun.heading)
                                                   : gPreviewRun.heading;
        if (legacyDeadEnd) {
          gPreviewRun.deadEndRaised = true;
        }
        startPreviewPhase(NavState::FrontNormalize, gPreviewRun.latchedEvent);
        return;
      }

      gPreviewRun.displayEvent = (frame.candidate == EventType::TJunctionStraight)
                                   ? EventType::TJunctionStraight
                                   : EventType::Corridor;
      return;
    }

    case NavState::ApproachWall:
      gPreviewRun.displayEvent = gPreviewRun.latchedEvent;
      if (!frame.ultrasonicValid) {
        return;
      }
      if (frame.front < gCfg.frontStopDistanceCm) {
        if (gPreviewRun.latchedEvent == EventType::TJunction) {
          gPreviewRun.pendingHeading = chooseHeadingForEvent(frame, gPreviewRun.latchedEvent, gPreviewRun.heading);
        }
        gPreviewRun.correctionConfirmEnabled = true;
        gPreviewRun.postCorrectionReleaseActive = false;
        startPreviewPhase(NavState::FrontNormalize, gPreviewRun.latchedEvent);
    ...
    case NavState::FrontNormalize: {
      gPreviewRun.displayEvent = gPreviewRun.latchedEvent;
      {
        EventType confirmed = EventType::Idle;
        bool leaveNormalize = false;
        bool finishNow = false;
        EventType correctionCandidate =
          correctionStageCandidate(frame, gPreviewRun.correctionConfirmEnabled);
        if (gPreviewRun.correctionConfirmEnabled &&
            updateEventConfirmState(gPreviewRun.eventConfirm, correctionCandidate, confirmed)) {
          (void)applyCompensationEventOverride(frame,
                                              confirmed,
                                              gPreviewRun.heading,
                                              gPreviewRun.latchedEvent,
                                              gPreviewRun.pendingHeading,
                                              gPreviewRun.finishArmed,
                                              gPreviewRun.tJunctionRaised,
                                              gPreviewRun.deadEndRaised,
                                              false,
                                              TurnChoice::Left,
                                              leaveNormalize,
          ...
          if (leaveNormalize) {
            resetDrivePidControl();
            gPreviewRun.postCorrectionReleaseActive = true;
            prepareOverallSpeedAfterStopEvent(gPreviewRun.latchedEvent);
            startPreviewPhase(NavState::AcquireCorridor, gPreviewRun.latchedEvent);
            return;
          }
        }
      }
      bool correctionEventReady =
        !gPreviewRun.correctionConfirmEnabled || gPreviewRun.latchedEvent != EventType::Corridor;
      if (frontNormalizationComplete(frame, gPreviewRun.latchedEvent) && correctionEventReady) {
        if (postCorrectionDeadEndTriggered(frame, gPreviewRun.latchedEvent)) {
          gPreviewRun.deadEndRaised = true;
          gPreviewRun.latchedEvent = EventType::DeadEnd;
          gPreviewRun.pendingHeading = oppositeOf(gPreviewRun.heading);
        }
        if (gPreviewRun.latchedEvent == EventType::Start) {
          gPreviewRun.heading = gPreviewRun.pendingHeading;
          gPreviewHeading = gPreviewRun.heading;
          initializeRuntimeOverallSpeed(true);
          gPreviewRun.postCorrectionReleaseActive = true;
          startPreviewPhase(NavState::AcquireCorridor, EventType::Corridor);
          return;
        }
        gPreviewRun.heading = gPreviewRun.pendingHeading;
        gPreviewHeading = gPreviewRun.heading;
    ...
    case NavState::AcquireCorridor:
      gPreviewRun.displayEvent = (gPreviewRun.latchedEvent == EventType::Start)
                                   ? EventType::Corridor
                                   : gPreviewRun.latchedEvent;
      if (holdPostCorrectionRelease(gPreviewRun.postCorrectionReleaseActive, frame)) {
        if (gPreviewRun.latchedEvent != EventType::Start &&
            updateCorridorConfirmState(gPreviewRun.corridorConfirm, frame.corridorSignature)) {
          gPreviewRun.postCorrectionReleaseActive = false;
          startPreviewPhase(NavState::Corridor, EventType::Corridor);
        }
        return;
      }
      if (gPreviewRun.latchedEvent == EventType::Start) {
        gPreviewRun.latchedEvent = EventType::Corridor;
        startPreviewPhase(NavState::Corridor, EventType::Corridor);
        return;
      }
      if (frame.ultrasonicValid && frame.front < gCfg.frontStopDistanceCm) {
      ...
      if (updateCorridorConfirmState(gPreviewRun.corridorConfirm, frame.corridorSignature)) {
        startPreviewPhase(NavState::Corridor, EventType::Corridor);
      }
      return;

    case NavState::Finished:
      gPreviewRun.displayEvent = EventType::Finish;
      return;
...
void updateExecutive() {
  ...
  updateOverallSpeedActualForState(gExecutive.state);
  ...
  switch (gExecutive.state) {
    case NavState::StartSeek:
      ...
      }
      {
        PerceptionFrame startFrame = frame;
        startFrame.corridorSignature = startSignatureForFrame(frame);
        if (updateCorridorConfirmation(startFrame)) {
          gExecutive.latchedEvent = EventType::Start;
          gExecutive.pendingHeading = gExecutive.heading;
          gExecutive.correctionConfirmEnabled = false;
          gExecutive.postCorrectionReleaseActive = false;
          enterPhase(NavState::FrontNormalize, EventType::Start);
          return;
        }
      }
    ...
    case NavState::Corridor: {
      ...
      if (frame.ultrasonicValid && frame.front < gCfg.frontStopDistanceCm) {
        bool legacyDeadEnd = legacyDeadEndStopTriggered(frame);
        resetEventConfirmation();
        gExecutive.correctionConfirmEnabled = !legacyDeadEnd;
        gExecutive.postCorrectionReleaseActive = false;
        gExecutive.latchedEvent = legacyDeadEnd ? EventType::DeadEnd : EventType::Corridor;
        gExecutive.pendingHeading = legacyDeadEnd ? oppositeOf(gExecutive.heading)
                                                  : gExecutive.heading;
        if (legacyDeadEnd) {
          gPlanner.currentAttemptHadDeadEnd = true;
          gExecutive.deadEndRaised = true;
        }
        enterPhase(NavState::FrontNormalize, gExecutive.latchedEvent);
        return;
      }

      if (frame.candidate == EventType::TJunctionStraight) {
        gExecutive.displayEvent = EventType::TJunctionStraight;
      } else {
    ...
    case NavState::ApproachWall:
      ...
      if (frame.front < gCfg.frontStopDistanceCm) {
        if (gExecutive.latchedEvent == EventType::TJunction) {
          gExecutive.pendingHeading = chooseHeadingForEvent(frame, gExecutive.latchedEvent, gExecutive.heading);
        }
        gExecutive.correctionConfirmEnabled = true;
        gExecutive.postCorrectionReleaseActive = false;
        enterPhase(NavState::FrontNormalize, gExecutive.latchedEvent);
    ...
    case NavState::FrontNormalize: {
      ...
      runFrontNormalization(gExecutive.heading, frame, gExecutive.latchedEvent);

      if (!frontNormalizationComplete(frame, gExecutive.latchedEvent)) {
        gExecutive.displayEvent = gExecutive.latchedEvent;
        return;
      }

      if (gExecutive.latchedEvent == EventType::Start) {
        gExecutive.heading = gExecutive.pendingHeading;
        resetDrivePidControl();
        initializeRuntimeOverallSpeed(true);
        gExecutive.postCorrectionReleaseActive = true;
        enterPhase(NavState::AcquireCorridor, EventType::Corridor);
        return;
      }

      if (postCorrectionDeadEndTriggered(frame, gExecutive.latchedEvent)) {
        gPlanner.currentAttemptHadDeadEnd = true;
        gExecutive.deadEndRaised = true;
        gExecutive.latchedEvent = EventType::DeadEnd;
        gExecutive.pendingHeading = oppositeOf(gExecutive.heading);
      }

      if (gExecutive.latchedEvent == EventType::DeadEnd) {
        gExecutive.heading = gExecutive.pendingHeading;
        resetDrivePidControl();
        gExecutive.postCorrectionReleaseActive = false;
        prepareOverallSpeedAfterStopEvent(EventType::DeadEnd);
        enterPhase(NavState::AcquireCorridor, EventType::DeadEnd);
        return;
      }

      EventType confirmed = EventType::Idle;
      EventType turnCandidate = postCorrectionTurnCandidate(frame);
      gExecutive.displayEvent = (turnCandidate == EventType::Idle) ? EventType::Corridor : turnCandidate;

      if (updateEventConfirmState(gExecutive.eventConfirm, turnCandidate, confirmed)) {
        ...
        resetDrivePidControl();
        gExecutive.postCorrectionReleaseActive = false;
        prepareOverallSpeedAfterStopEvent(confirmed);
        enterPhase(NavState::AcquireCorridor, confirmed);
      }
      return;
    }

    case NavState::AcquireCorridor:
      gExecutive.displayEvent = (gExecutive.latchedEvent == EventType::Start)
                                  ? EventType::Corridor
                                  : gExecutive.latchedEvent;
      if (holdPostCorrectionRelease(gExecutive.postCorrectionReleaseActive, frame)) {
        if (gExecutive.latchedEvent != EventType::Start &&
            updateCorridorConfirmation(frame)) {
          gExecutive.postCorrectionReleaseActive = false;
          resetDrivePidControl();
      ...
      if (gExecutive.latchedEvent == EventType::Start) {
        gExecutive.latchedEvent = EventType::Corridor;
        resetDrivePidControl();
        enterPhase(NavState::Corridor, EventType::Corridor);
        return;
      }
...
static bool applyCompensationEventOverride(const PerceptionFrame& frame,
                                           ...
                                           bool& leaveNormalize,
                                           bool& finishNow) {
  leaveNormalize = false;
  finishNow = false;

  if (confirmedEvent == EventType::Idle || confirmedEvent == latchedEvent) {
    return false;
  }

  switch (confirmedEvent) {
    case EventType::Finish:
      latchedEvent = confirmedEvent;
    ...
    case EventType::DeadEnd:
      latchedEvent = confirmedEvent;
      deadEndRaised = true;
      pendingHeading = oppositeOf(currentHeading);
      return true;

    case EventType::LeftTurn:
    case EventType::RightTurn:
      latchedEvent = confirmedEvent;
      pendingHeading = chooseHeadingForEvent(frame, confirmedEvent, currentHeading);
      return true;

    case EventType::TJunction:
      latchedEvent = confirmedEvent;
      finishArmed = true;
      tJunctionRaised = true;
      if (preferConfiguredTJunction) {
        pendingHeading = (configuredTurn == TurnChoice::Left)
...
void updatePidTestDrive() {
  ...
  followSmartForced(gPidTestHeading, gCfg.baseSpeed, pidSourceForTestMode(gPidTestMode));
}
...
void startEventTestRun() {
  ...
  gEventTestRun.correctionConfirmEnabled = true;
  gEventTestRun.postCorrectionReleaseActive = false;
  ...
  initializeRuntimeOverallSpeed(gSelectedEventTest == EventTestCase::Start);
  gEventTestRun.eventConfirm.active = false;
  gEventTestRun.corridorConfirm.active = false;
  resetDrivePidControl();
  resetImuPid();
...
void handleRootInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gRootIndex) {
    ...
    case 6:
      ...
      startPreviewExecution();
      gCurrentScreen = Screen::MazeTestRun;
      break;
...
void handleStartSettingsInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gStartSettingsIndex) {
    ...
    case 1:
      ...
      break;
    case 2:
      openDigitEditor("Overall start", "x", &workingSettings.overallStart_x100, 0, 100,
                      "Initial overall speed", "while Start is active", Screen::StartSettings, 1, 2);
      break;
    case 3:
      openDigitEditor("Overall ramp", "x/s", &workingSettings.overallRampK_x100, 0, 9999,
                      "Ramp rate after", "leaving Start state", Screen::StartSettings, 2, 2);
      break;
    case 4:
      gCurrentScreen = Screen::Root;
      break;
...
void handleSettingsInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gSettingsIndex) {
    ...
    case 4:
      workingSettings.leftTurnRampEnabled = workingSettings.leftTurnRampEnabled ? 0 : 1;
      commitRuntimeConfig();
      break;
    case 5:
      workingSettings.rightTurnRampEnabled = workingSettings.rightTurnRampEnabled ? 0 : 1;
      commitRuntimeConfig();
      break;
    case 6:
      workingSettings.tJunctionRampEnabled = workingSettings.tJunctionRampEnabled ? 0 : 1;
      commitRuntimeConfig();
      break;
    case 7:
      workingSettings.deadEndRampEnabled = workingSettings.deadEndRampEnabled ? 0 : 1;
      commitRuntimeConfig();
      break;
    case 8: openDigitEditor("Front stop", "cm", &workingSettings.frontStopDistance_x100, 0, 9999, "Front distance that", "starts hold stage", Screen::Settings, 2, 2); break;
    case 9: openDigitEditor("Front hold", "cm", &workingSettings.frontHoldReference_x100, 0, 9999, "Front distance held", "before heading swap", Screen::Settings, 2, 2); break;
    case 10: openDigitEditor("Front P", "x", &workingSettings.frontPidP_x1000, 0, 9999, "Front-hold proportional", "gain", Screen::Settings, 1, 3); break;
    case 11: openDigitEditor("Front I", "x", &workingSettings.frontPidI_x1000, 0, 9999, "Front-hold integral", "gain", Screen::Settings, 1, 3); break;
    case 12: openDigitEditor("Front D", "x", &workingSettings.frontPidD_x1000, 0, 9999, "Front-hold derivative", "gain", Screen::Settings, 1, 3); break;
    case 13: openDigitEditor("Turn yaw", "deg", &workingSettings.turnYawTolerance_x100, 0, 9999, "Yaw window before", "heading swap", Screen::Settings, 2, 2); break;
    case 14: openDigitEditor("Wall thr", "cm", &workingSettings.corridorWallThreshold_x100, 0, 9999, "Wall limit for PID", "and finish detection", Screen::Settings, 2, 2); break;
    case 15: openDigitEditor("Turn detect", "cm", &workingSettings.turnDetectDistance_x100, 0, 9999, "Upper front range", "that still means turn", Screen::Settings, 2, 2); break;
    case 16: openDigitEditor("Dead-end", "cm", &workingSettings.deadEndDistance_x100, 0, 9999, "Front distance limit", "for dead-end event", Screen::Settings, 2, 2); break;
    case 17: openDigitEditor("Finish", "cm", &workingSettings.finishDistance_x100, 0, 9999, "Front distance needed", "before finish counts", Screen::Settings, 2, 2); break;
    case 18: openDigitEditor("Check gap", "s", &workingSettings.sensingInterval_x100, 1, 9999, "Time between event", "verification checks", Screen::Settings, 2, 2); break;
    case 19: openDigitEditor("US avg k", "x", &workingSettings.ultrasonicAvgCount, 1, ULTRASONIC_AVG_COUNT_MAX, "How many ultrasonic", "samples are averaged", Screen::Settings, 1, 0); break;
    case 20: openDigitEditor("Match N", "x", &workingSettings.eventConfirmCount, 1, 9, "How many identical", "events confirm state", Screen::Settings, 1, 0); break;
    case 21: gCurrentScreen = Screen::Root; break;
    default: break;
  }
}

void handlePidInput(int encoderDelta, ButtonEvent buttonEvent) {
  int itemCount = (activeSettings.pid1WallSymmetryEnabled != 0) ? 19 : 22;
  if (encoderDelta != 0) {
  ...
  int index = 0;
  if (gPidIndex == index++) {
    workingSettings.wallPidEnabled = workingSettings.wallPidEnabled ? 0 : 1;
    commitRuntimeConfig();
    return;
  }
  if (gPidIndex == index++) {
  ...
  if (gPidIndex == index++) {
    openDigitEditor("2W P", "x", &workingSettings.p_x1000, 0, 9999, "2-wall proportional", "gain", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("2W I", "x", &workingSettings.i_x1000, 0, 9999, "2-wall integral", "gain", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("2W D", "x", &workingSettings.d_x1000, 0, 9999, "2-wall derivative", "gain", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("Curve b", "x", &workingSettings.curveB_x100, 1, 9999, "Shapes small errors", "before PID acts", Screen::PID, 1, 2);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("1-wall", "cm", &workingSettings.pidWallDistance_x100, 0, 9999, "Target distance for", "1-wall mode", Screen::PID, 2, 2);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("1W P", "x", &workingSettings.pid1WallP_x1000, 0, 9999, "1-wall proportional", "gain", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("1W I", "x", &workingSettings.pid1WallI_x1000, 0, 9999, "1-wall integral", "gain", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("1W D", "x", &workingSettings.pid1WallD_x1000, 0, 9999, "1-wall derivative", "gain", Screen::PID, 1, 3);
    return;
  }
  if (workingSettings.pid1WallSymmetryEnabled == 0) {
    if (gPidIndex == index++) {
      openDigitEditor("1WR P", "x", &workingSettings.pid1WallRightP_x1000, 0, 9999, "Right-wall proportional", "gain", Screen::PID, 1, 3);
      return;
    }
    if (gPidIndex == index++) {
      openDigitEditor("1WR I", "x", &workingSettings.pid1WallRightI_x1000, 0, 9999, "Right-wall integral", "gain", Screen::PID, 1, 3);
      return;
    }
    if (gPidIndex == index++) {
      openDigitEditor("1WR D", "x", &workingSettings.pid1WallRightD_x1000, 0, 9999, "Right-wall derivative", "gain", Screen::PID, 1, 3);
      return;
    }
  }
  if (gPidIndex == index++) {
    openDigitEditor("1W dead", "cm", &workingSettings.pidDeadband1W_x100, 0, 9999, "Deadband for single", "wall distance error", Screen::PID, 2, 2);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("2W full", "cm", &workingSettings.pidTwoWallFullError_x100, 1, 9999, "Side difference that", "means full correction", Screen::PID, 2, 2);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("2W dead", "x", &workingSettings.pidDeadband2W_x1000, 0, 1000, "Deadband for scaled", "two-wall error", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("D alpha", "x", &workingSettings.pidDFilterAlpha_x1000, 10, 1000, "Derivative filter", "smoothing alpha", Screen::PID, 1, 3);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("Left out", "x", &workingSettings.pidLeftScale_x100, 10, 300, "Extra scale for", "left correction", Screen::PID, 1, 2);
    return;
  }
  if (gPidIndex == index++) {
    openDigitEditor("Right out", "x", &workingSettings.pidRightScale_x100, 10, 300, "Extra scale for", "right correction", Screen::PID, 1, 2);
    return;
  }
  if (gPidIndex == index++) {
    gPidTestHeading = headingFromIndex(activeSettings.startHeadingIndex);
    gPidTestMode = PidTestMode::TwoWall;
    syncOverallSpeedActualToTarget();
    gCurrentScreen = Screen::PIDTest;
    return;
  }
  if (gPidIndex == index++) {
...
void handleImuPidMenuInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gImuPidMenuIndex) {
    ...
    case 4: openDigitEditor("Lat P", "x", &workingSettings.imuLatP_x10000, 0, 99999, "No-wall lateral", "proportional gain", Screen::IMUPIDMenu, 1, 4); break;
    case 5: openDigitEditor("Lat I", "x", &workingSettings.imuLatI_x10000, 0, 99999, "No-wall lateral", "integral gain", Screen::IMUPIDMenu, 1, 4); break;
    case 6: openDigitEditor("Lat D", "x", &workingSettings.imuLatD_x10000, 0, 99999, "No-wall lateral", "derivative gain", Screen::IMUPIDMenu, 1, 4); break;
    case 7: openDigitEditor("Lat LPF", "x", &workingSettings.imuLatFilterAlpha_x100, 1, 100, "Low-pass alpha for", "lateral accel", Screen::IMUPIDMenu, 1, 2); break;
    case 8: openDigitEditor("IMU thr", "deg", &workingSettings.imuAngleThreshold_x100, 0, 4500, "Extra angle above", "sensor deadband", Screen::IMUPIDMenu, 2, 2); break;
    case 9: openDigitEditor("Yaw err", "deg", &workingSettings.imuPidTestYawError_x100, 0, 4500, "If yaw stays inside", "this range stop", Screen::IMUPIDMenu, 2, 2); break;
    case 10:
...
void handlePidTestInput(int encoderDelta, ButtonEvent buttonEvent) {
  if (encoderDelta != 0) {
    int nextMode = (int)gPidTestMode + encoderDelta;
    if (nextMode < 0) nextMode = 0;
    if (nextMode > (int)PidTestMode::NoWall) nextMode = (int)PidTestMode::NoWall;
    PidTestMode newMode = (PidTestMode)nextMode;
    if (newMode != gPidTestMode) {
      gPidTestMode = newMode;
      resetDrivePidControl();
    }
  }
  ...
  if (buttonEvent == ButtonEvent::ShortPress) {
    switch (gMotorTuneIndex) {
      ...
      case 9:
        ...
        break;
      case 10:
        openDigitEditor("FL stop", "deg", &workingSettings.servoStopFL_x1000, 0, 180000,
                        "Front-left neutral", "center output", Screen::MotorTune, 3, 3);
        break;
      case 11:
        openDigitEditor("FR stop", "deg", &workingSettings.servoStopFR_x1000, 0, 180000,
                        "Front-right neutral", "center output", Screen::MotorTune, 3, 3);
        break;
      case 12:
        openDigitEditor("RR stop", "deg", &workingSettings.servoStopRR_x1000, 0, 180000,
                        "Rear-right neutral", "center output", Screen::MotorTune, 3, 3);
        break;
      case 13:
        openDigitEditor("RL stop", "deg", &workingSettings.servoStopRL_x1000, 0, 180000,
                        "Rear-left neutral", "center output", Screen::MotorTune, 3, 3);
        break;
      case 14:
...
static bool eventConfigFieldRelevant(EventTestCase testCase, EventConfigField field) {
  switch (field) {
    ...
    case EventConfigField::WallToggle:
    ...
    case EventConfigField::OneWallP:
    case EventConfigField::OneWallI:
    case EventConfigField::OneWallD:
    ...
    case EventConfigField::ImuLatP:
    case EventConfigField::ImuLatI:
    case EventConfigField::ImuLatD:
    case EventConfigField::ImuLatFilterAlpha:
    ...
    case EventConfigField::Back:
      return true;
    case EventConfigField::OverallStart:
    case EventConfigField::OverallRampK:
      return testCase == EventTestCase::Start;
    case EventConfigField::LeftTurnRamp:
      return testCase == EventTestCase::LeftTurn ||
             testCase == EventTestCase::Random;
    case EventConfigField::RightTurnRamp:
      return testCase == EventTestCase::RightTurn ||
             testCase == EventTestCase::Random;
    case EventConfigField::TJunctionRamp:
      return testCase == EventTestCase::TJunction ||
             testCase == EventTestCase::Random;
    case EventConfigField::DeadEndRamp:
      return testCase == EventTestCase::DeadEnd ||
             testCase == EventTestCase::Random;
    case EventConfigField::OneWallRightP:
    case EventConfigField::OneWallRightI:
    case EventConfigField::OneWallRightD:
      return workingSettings.pid1WallSymmetryEnabled == 0;
    ...
    case EventConfigField::FrontStop:
      return testCase == EventTestCase::LeftTurn ||
             ...
             testCase == EventTestCase::Random;
    case EventConfigField::FrontHoldRef:
    case EventConfigField::FrontPidP:
    case EventConfigField::FrontPidI:
    case EventConfigField::FrontPidD:
    case EventConfigField::TurnYawTol:
      return testCase == EventTestCase::LeftTurn ||
             testCase == EventTestCase::RightTurn ||
             testCase == EventTestCase::TJunction ||
             testCase == EventTestCase::Random;
...
static const char* eventConfigDesc1(EventConfigField field) {
  switch (field) {
    ...
    case EventConfigField::OverallStart: return "Initial overall speed";
    case EventConfigField::OverallRampK: return "Overall speed ramp";
    case EventConfigField::LeftTurnRamp: return "Ramp overall speed";
    case EventConfigField::RightTurnRamp: return "Ramp overall speed";
    case EventConfigField::TJunctionRamp: return "Ramp overall speed";
    case EventConfigField::DeadEndRamp: return "Ramp overall speed";
    case EventConfigField::FrontStop: return "Front distance to";
    case EventConfigField::FrontHoldRef: return "Front distance held";
    case EventConfigField::FrontPidP: return "Front-hold proportional";
    case EventConfigField::FrontPidI: return "Front-hold integral";
    case EventConfigField::FrontPidD: return "Front-hold derivative";
    case EventConfigField::TurnYawTol: return "Yaw tolerance before";
    ...
    case EventConfigField::TurnWait: return "Unused legacy";
    case EventConfigField::CheckGap: return "Time between";
    case EventConfigField::MatchCount: return "How many identical";
    case EventConfigField::WallToggle: return "Enable or disable";
    case EventConfigField::OneWallSymmetry: return "Use one shared";
    case EventConfigField::WallP: return "2-wall proportional";
    case EventConfigField::WallI: return "2-wall integral";
    case EventConfigField::WallD: return "2-wall derivative";
    case EventConfigField::CurveB: return "Wall error shaping";
    case EventConfigField::OneWallTarget: return "1-wall target";
    case EventConfigField::OneWallP: return "1-wall proportional";
    case EventConfigField::OneWallI: return "1-wall integral";
    case EventConfigField::OneWallD: return "1-wall derivative";
    case EventConfigField::OneWallRightP: return "Right-wall proportional";
    case EventConfigField::OneWallRightI: return "Right-wall integral";
    case EventConfigField::OneWallRightD: return "Right-wall derivative";
    ...
    case EventConfigField::ImuLatP: return "No-wall lateral P";
    case EventConfigField::ImuLatI: return "No-wall lateral I";
    case EventConfigField::ImuLatD: return "No-wall lateral D";
    case EventConfigField::ImuLatFilterAlpha: return "No-wall lateral LPF";
...
static const char* eventConfigDesc2(EventConfigField field) {
  switch (field) {
    ...
    case EventConfigField::OverallStart: return "while Start is active.";
    case EventConfigField::OverallRampK: return "after Start [x/s].";
    case EventConfigField::LeftTurnRamp: return "after left-stop release.";
    case EventConfigField::RightTurnRamp: return "after right-stop release.";
    case EventConfigField::TJunctionRamp: return "after T-stop release.";
    case EventConfigField::DeadEndRamp: return "after dead-end release.";
    case EventConfigField::FrontStop: return "enter stop stage.";
    case EventConfigField::FrontHoldRef: return "before heading swap.";
    case EventConfigField::FrontPidP: return "gain for front hold.";
    case EventConfigField::FrontPidI: return "gain for front hold.";
    case EventConfigField::FrontPidD: return "gain for front hold.";
    case EventConfigField::TurnYawTol: return "heading swap [deg].";
    ...
    case EventConfigField::TurnWait: return "field hidden now.";
    case EventConfigField::CheckGap: return "event rechecks [s].";
    case EventConfigField::MatchCount: return "reads must match.";
    case EventConfigField::WallToggle: return "wall PID globally.";
    case EventConfigField::OneWallSymmetry: return "PID for both walls.";
    case EventConfigField::WallP: return "gain for 2-wall mode.";
    case EventConfigField::WallI: return "gain for 2-wall drift.";
    case EventConfigField::WallD: return "gain for 2-wall damping.";
    case EventConfigField::CurveB: return "before PID acts.";
    case EventConfigField::OneWallTarget: return "distance [cm].";
    case EventConfigField::OneWallP: return "gain for 1-wall mode.";
    case EventConfigField::OneWallI: return "gain for 1-wall drift.";
    case EventConfigField::OneWallD: return "gain for 1-wall damping.";
    case EventConfigField::OneWallRightP: return "gain if symmetry off.";
    case EventConfigField::OneWallRightI: return "gain if symmetry off.";
    case EventConfigField::OneWallRightD: return "gain if symmetry off.";
    ...
    case EventConfigField::ImuLatP: return "gain for side drift.";
    case EventConfigField::ImuLatI: return "gain for side bias.";
    case EventConfigField::ImuLatD: return "gain for side damping.";
    case EventConfigField::ImuLatFilterAlpha: return "input filter alpha.";
...
void handleEventTestConfigInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (currentEventConfigFieldAt(gEventTestConfigIndex)) {
    ...
    case EventConfigField::OverallSpeed:
      ...
      break;
    case EventConfigField::OverallStart:
      openDigitEditor("Overall start", "x", &workingSettings.overallStart_x100, 0, 100, eventConfigDesc1(EventConfigField::OverallStart), eventConfigDesc2(EventConfigField::OverallStart), Screen::EventTestConfig, 1, 2);
      break;
    case EventConfigField::OverallRampK:
      openDigitEditor("Overall ramp", "x/s", &workingSettings.overallRampK_x100, 0, 9999, eventConfigDesc1(EventConfigField::OverallRampK), eventConfigDesc2(EventConfigField::OverallRampK), Screen::EventTestConfig, 2, 2);
      break;
    case EventConfigField::LeftTurnRamp:
      workingSettings.leftTurnRampEnabled = workingSettings.leftTurnRampEnabled ? 0 : 1;
      commitRuntimeConfig();
    ...
    case EventConfigField::FrontStop:
      ...
      break;
    case EventConfigField::FrontHoldRef:
      openDigitEditor("Front hold", "cm", &workingSettings.frontHoldReference_x100, 0, 9999, eventConfigDesc1(EventConfigField::FrontHoldRef), eventConfigDesc2(EventConfigField::FrontHoldRef), Screen::EventTestConfig, 2, 2);
      break;
    case EventConfigField::FrontPidP:
      openDigitEditor("Front P", "x", &workingSettings.frontPidP_x1000, 0, 9999, eventConfigDesc1(EventConfigField::FrontPidP), eventConfigDesc2(EventConfigField::FrontPidP), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::FrontPidI:
      openDigitEditor("Front I", "x", &workingSettings.frontPidI_x1000, 0, 9999, eventConfigDesc1(EventConfigField::FrontPidI), eventConfigDesc2(EventConfigField::FrontPidI), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::FrontPidD:
      openDigitEditor("Front D", "x", &workingSettings.frontPidD_x1000, 0, 9999, eventConfigDesc1(EventConfigField::FrontPidD), eventConfigDesc2(EventConfigField::FrontPidD), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::TurnYawTol:
      openDigitEditor("Turn yaw", "deg", &workingSettings.turnYawTolerance_x100, 0, 9999, eventConfigDesc1(EventConfigField::TurnYawTol), eventConfigDesc2(EventConfigField::TurnYawTol), Screen::EventTestConfig, 2, 2);
      break;
    case EventConfigField::WallThreshold:
    ...
    case EventConfigField::WallP:
      ...
      break;
    case EventConfigField::WallI:
      openDigitEditor("2W I", "x", &workingSettings.i_x1000, 0, 9999, eventConfigDesc1(EventConfigField::WallI), eventConfigDesc2(EventConfigField::WallI), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::WallD:
      openDigitEditor("2W D", "x", &workingSettings.d_x1000, 0, 9999, eventConfigDesc1(EventConfigField::WallD), eventConfigDesc2(EventConfigField::WallD), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::CurveB:
    ...
    case EventConfigField::OneWallTarget:
      ...
      break;
    case EventConfigField::OneWallP:
      openDigitEditor("1W P", "x", &workingSettings.pid1WallP_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallP), eventConfigDesc2(EventConfigField::OneWallP), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallI:
      openDigitEditor("1W I", "x", &workingSettings.pid1WallI_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallI), eventConfigDesc2(EventConfigField::OneWallI), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallD:
      openDigitEditor("1W D", "x", &workingSettings.pid1WallD_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallD), eventConfigDesc2(EventConfigField::OneWallD), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallRightP:
      openDigitEditor("1WR P", "x", &workingSettings.pid1WallRightP_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallRightP), eventConfigDesc2(EventConfigField::OneWallRightP), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallRightI:
      openDigitEditor("1WR I", "x", &workingSettings.pid1WallRightI_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallRightI), eventConfigDesc2(EventConfigField::OneWallRightI), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallRightD:
      openDigitEditor("1WR D", "x", &workingSettings.pid1WallRightD_x1000, 0, 9999, eventConfigDesc1(EventConfigField::OneWallRightD), eventConfigDesc2(EventConfigField::OneWallRightD), Screen::EventTestConfig, 1, 3);
      break;
    case EventConfigField::OneWallDeadband:
    ...
    case EventConfigField::ImuD:
      ...
      break;
    case EventConfigField::ImuLatP:
      openDigitEditor("Lat P", "x", &workingSettings.imuLatP_x10000, 0, 99999, eventConfigDesc1(EventConfigField::ImuLatP), eventConfigDesc2(EventConfigField::ImuLatP), Screen::EventTestConfig, 1, 4);
      break;
    case EventConfigField::ImuLatI:
      openDigitEditor("Lat I", "x", &workingSettings.imuLatI_x10000, 0, 99999, eventConfigDesc1(EventConfigField::ImuLatI), eventConfigDesc2(EventConfigField::ImuLatI), Screen::EventTestConfig, 1, 4);
      break;
    case EventConfigField::ImuLatD:
      openDigitEditor("Lat D", "x", &workingSettings.imuLatD_x10000, 0, 99999, eventConfigDesc1(EventConfigField::ImuLatD), eventConfigDesc2(EventConfigField::ImuLatD), Screen::EventTestConfig, 1, 4);
      break;
    case EventConfigField::ImuLatFilterAlpha:
      openDigitEditor("Lat LPF", "x", &workingSettings.imuLatFilterAlpha_x100, 1, 100, eventConfigDesc1(EventConfigField::ImuLatFilterAlpha), eventConfigDesc2(EventConfigField::ImuLatFilterAlpha), Screen::EventTestConfig, 1, 2);
      break;
    case EventConfigField::ImuThreshold:
...
void updateEventTestRun() {
  ...
  updateOverallSpeedActualForState(gEventTestRun.state);
  ...
  switch (gEventTestRun.state) {
    case NavState::StartSeek:
      ...
      if (updateCorridorConfirmState(gEventTestRun.corridorConfirm, startSignatureForFrame(frame))) {
        gEventTestRun.latchedEvent = EventType::Start;
        gEventTestRun.pendingHeading = gEventTestRun.heading;
        gEventTestRun.correctionConfirmEnabled = false;
        gEventTestRun.postCorrectionReleaseActive = false;
        startEventPhase(NavState::FrontNormalize, EventType::Start);
        return;
      }
    ...
    case NavState::Corridor: {
      ...
      EventType confirmed = EventType::Idle;
      if (eventTriggersEnabledForState(gEventTestRun.state) &&
        ...
        if (confirmed == EventType::DeadEnd) {
          ...
          gEventTestRun.correctionConfirmEnabled = false;
          gEventTestRun.postCorrectionReleaseActive = false;
          startEventPhase(NavState::FrontNormalize, EventType::DeadEnd);
          return;
        }
        if (confirmed == EventType::TJunctionStraight) {
          gEventTestRun.displayEvent = EventType::TJunctionStraight;
          return;
        }
        if (confirmed == EventType::LeftTurn ||
            confirmed == EventType::RightTurn ||
      ...
      if (frame.ultrasonicValid && frame.front < gCfg.frontStopDistanceCm) {
        bool legacyDeadEnd = legacyDeadEndStopTriggered(frame);
        gEventTestRun.correctionConfirmEnabled = !legacyDeadEnd;
        gEventTestRun.postCorrectionReleaseActive = false;
        gEventTestRun.latchedEvent = legacyDeadEnd ? EventType::DeadEnd : EventType::Corridor;
        gEventTestRun.pendingHeading = legacyDeadEnd ? oppositeOf(gEventTestRun.heading)
                                                     : gEventTestRun.heading;
        if (legacyDeadEnd) {
          gEventTestRun.deadEndRaised = true;
        }
        startEventPhase(NavState::FrontNormalize, gEventTestRun.latchedEvent);
        return;
      }

      if (frame.candidate == EventType::TJunctionStraight) {
        gEventTestRun.displayEvent = EventType::TJunctionStraight;
      } else if (gSelectedEventTest == EventTestCase::Random) {
        gEventTestRun.displayEvent = EventType::Corridor;
      }
      followSmart(gEventTestRun.heading, gCfg.baseSpeed);
      return;
    }
    ...
    case NavState::ApproachWall:
      ...
      if (frame.front < gCfg.frontStopDistanceCm) {
        if (gEventTestRun.latchedEvent == EventType::TJunction) {
          if (gSelectedEventTest == EventTestCase::TJunction) {
            gEventTestRun.pendingHeading = (currentEventTestConfig().turnChoice == TurnChoice::Left)
                                             ? turnLeftOf(gEventTestRun.heading)
                                             : turnRightOf(gEventTestRun.heading);
          } else {
            gEventTestRun.pendingHeading = chooseHeadingForEvent(frame, gEventTestRun.latchedEvent, gEventTestRun.heading);
          }
        }
        gEventTestRun.correctionConfirmEnabled = true;
        gEventTestRun.postCorrectionReleaseActive = false;
    ...
    case NavState::FrontNormalize: {
      ...
      }
      {
        ...
        bool finishNow = false;
        EventType correctionCandidate =
          correctionStageCandidate(frame, gEventTestRun.correctionConfirmEnabled);
        if (gEventTestRun.correctionConfirmEnabled &&
            updateEventConfirmState(gEventTestRun.eventConfirm, correctionCandidate, confirmed)) {
          (void)applyCompensationEventOverride(frame,
                                              confirmed,
                                              gEventTestRun.heading,
                                              gEventTestRun.latchedEvent,
                                              gEventTestRun.pendingHeading,
                                              gEventTestRun.finishArmed,
                                              gEventTestRun.tJunctionRaised,
                                              gEventTestRun.deadEndRaised,
                                              gSelectedEventTest == EventTestCase::TJunction,
                                              currentEventTestConfig().turnChoice,
                                              leaveNormalize,
                                              finishNow);
          gEventTestRun.displayEvent = gEventTestRun.latchedEvent;
          if (finishNow) {
            startEventPhase(NavState::Finished, EventType::Finish);
            stopAll();
            return;
          }
          if (leaveNormalize) {
            resetDrivePidControl();
            gEventTestRun.postCorrectionReleaseActive = true;
            prepareOverallSpeedAfterStopEvent(gEventTestRun.latchedEvent);
            startEventPhase(NavState::AcquireCorridor, gEventTestRun.latchedEvent);
            return;
          }
        }
      }
      runFrontNormalization(gEventTestRun.heading, frame, gEventTestRun.latchedEvent);
      bool correctionEventReady =
        !gEventTestRun.correctionConfirmEnabled || gEventTestRun.latchedEvent != EventType::Corridor;
      if (frontNormalizationComplete(frame, gEventTestRun.latchedEvent) && correctionEventReady) {
        if (postCorrectionDeadEndTriggered(frame, gEventTestRun.latchedEvent)) {
          gEventTestRun.deadEndRaised = true;
          gEventTestRun.latchedEvent = EventType::DeadEnd;
          gEventTestRun.pendingHeading = oppositeOf(gEventTestRun.heading);
        }
        if (gEventTestRun.latchedEvent == EventType::Start) {
          gEventTestRun.heading = gEventTestRun.pendingHeading;
          resetDrivePidControl();
          initializeRuntimeOverallSpeed(true);
          gEventTestRun.postCorrectionReleaseActive = true;
          startEventPhase(NavState::AcquireCorridor, EventType::Corridor);
          return;
        }
        gEventTestRun.heading = gEventTestRun.pendingHeading;
        resetDrivePidControl();
    ...
    case NavState::AcquireCorridor:
      gEventTestRun.displayEvent = (gEventTestRun.latchedEvent == EventType::Start)
                                     ? EventType::Corridor
                                     : gEventTestRun.latchedEvent;
      if (holdPostCorrectionRelease(gEventTestRun.postCorrectionReleaseActive, frame)) {
        if (gEventTestRun.latchedEvent != EventType::Start &&
            updateCorridorConfirmState(gEventTestRun.corridorConfirm, frame.corridorSignature)) {
          gEventTestRun.postCorrectionReleaseActive = false;
      ...
      if (gEventTestRun.latchedEvent == EventType::Start) {
        gEventTestRun.latchedEvent = EventType::Corridor;
        resetDrivePidControl();
        startEventPhase(NavState::Corridor, EventType::Corridor);
        return;
      }
...
void handleIdleUiInput(int encoderDelta, ButtonEvent buttonEvent) {
  switch (gCurrentScreen) {
    ...
    case Screen::PIDTest:              handlePidTestInput(encoderDelta, buttonEvent); break;
...
void drawStartSettingsScreen() {
  ...
  snprintf(line2, sizeof(line2), "Overall St:%d.%02d",
           activeSettings.overallStart_x100 / 100,
           activeSettings.overallStart_x100 % 100);
  snprintf(line3, sizeof(line3), "Ramp K:%d.%02d",
           activeSettings.overallRampK_x100 / 100,
           activeSettings.overallRampK_x100 % 100);
  ...
  drawSimpleMenu("Start Settings", items, 5, gStartSettingsIndex);
  u8g2.setFont(u8g2_font_5x8_tf);
...
void drawSettingsScreen() {
  ...
  snprintf(items[4], sizeof(items[4]), "LTurnRamp:%s", activeSettings.leftTurnRampEnabled ? "ON" : "OFF");
  snprintf(items[5], sizeof(items[5]), "RTurnRamp:%s", activeSettings.rightTurnRampEnabled ? "ON" : "OFF");
  snprintf(items[6], sizeof(items[6]), "TJctRamp:%s", activeSettings.tJunctionRampEnabled ? "ON" : "OFF");
  snprintf(items[7], sizeof(items[7]), "DEndRamp:%s", activeSettings.deadEndRampEnabled ? "ON" : "OFF");
  formatCompactValue(value, sizeof(value), activeSettings.frontStopDistance_x100, 2);
  snprintf(items[8], sizeof(items[8]), "FrontStop:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontHoldReference_x100, 2);
  snprintf(items[9], sizeof(items[9]), "FrontRef:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidP_x1000, 3);
  snprintf(items[10], sizeof(items[10]), "FrontP:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidI_x1000, 3);
  snprintf(items[11], sizeof(items[11]), "FrontI:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidD_x1000, 3);
  snprintf(items[12], sizeof(items[12]), "FrontD:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.turnYawTolerance_x100, 2);
  snprintf(items[13], sizeof(items[13]), "YawTol:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.corridorWallThreshold_x100, 2);
  snprintf(items[14], sizeof(items[14]), "WallThr:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.turnDetectDistance_x100, 2);
  snprintf(items[15], sizeof(items[15]), "TurnDet:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.deadEndDistance_x100, 2);
  snprintf(items[16], sizeof(items[16]), "DeadEnd:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.finishDistance_x100, 2);
  snprintf(items[17], sizeof(items[17]), "Finish:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.sensingInterval_x100, 2);
  snprintf(items[18], sizeof(items[18]), "CheckGap:%s", value);
  snprintf(items[19], sizeof(items[19]), "USavg:%d", activeSettings.ultrasonicAvgCount);
  snprintf(items[20], sizeof(items[20]), "Match N:%d", activeSettings.eventConfirmCount);
  snprintf(items[21], sizeof(items[21]), "Back");
  const char* ptrs[22];
  for (int i = 0; i < 22; ++i) ptrs[i] = items[i];
  drawScrollableItemList(ptrs, 22, gSettingsIndex, 26);
  u8g2.setFont(u8g2_font_5x8_tf);
  u8g2.drawStr(0, 63, "Long: root");
}
...
void drawPIDScreen() {
  ...
  char value[16];
  int itemCount = 0;
  snprintf(items[itemCount++], sizeof(items[0]), "WallPID:%s", activeSettings.wallPidEnabled ? "ON" : "OFF");
  snprintf(items[itemCount++], sizeof(items[0]), "1WSym:%s", activeSettings.pid1WallSymmetryEnabled ? "ON" : "OFF");
  formatCompactValue(value, sizeof(value), activeSettings.p_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "2WP:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.i_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "2WI:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.d_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "2WD:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.curveB_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "Curve:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidWallDistance_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "1Wall:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pid1WallP_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "1WP:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pid1WallI_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "1WI:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pid1WallD_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "1WD:%s", value);
  if (!activeSettings.pid1WallSymmetryEnabled) {
    formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightP_x1000, 3);
    snprintf(items[itemCount++], sizeof(items[0]), "1WRP:%s", value);
    formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightI_x1000, 3);
    snprintf(items[itemCount++], sizeof(items[0]), "1WRI:%s", value);
    formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightD_x1000, 3);
    snprintf(items[itemCount++], sizeof(items[0]), "1WRD:%s", value);
  }
  formatCompactValue(value, sizeof(value), activeSettings.pidDeadband1W_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "1WDead:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidTwoWallFullError_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "2WFull:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidDeadband2W_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "2WDead:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidDFilterAlpha_x1000, 3);
  snprintf(items[itemCount++], sizeof(items[0]), "DAlpha:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidLeftScale_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "LeftOut:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.pidRightScale_x100, 2);
  snprintf(items[itemCount++], sizeof(items[0]), "RightOut:%s", value);
  snprintf(items[itemCount++], sizeof(items[0]), "PID Test");
  snprintf(items[itemCount++], sizeof(items[0]), "IMU PID");
  snprintf(items[itemCount++], sizeof(items[0]), "Back");
  const char* ptrs[24];
  for (int i = 0; i < itemCount; ++i) ptrs[i] = items[i];
  drawScrollableItemList(ptrs, itemCount, gPidIndex, 26);
  u8g2.setFont(u8g2_font_5x8_tf);
...
void drawImuPidMenuScreen() {
  ...
  formatCompactValue(value, sizeof(value), activeSettings.imuLatP_x10000, 4);
  snprintf(items[4], sizeof(items[4]), "LatP:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.imuLatI_x10000, 4);
  snprintf(items[5], sizeof(items[5]), "LatI:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.imuLatD_x10000, 4);
  snprintf(items[6], sizeof(items[6]), "LatD:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.imuLatFilterAlpha_x100, 2);
  snprintf(items[7], sizeof(items[7]), "LatA:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.imuAngleThreshold_x100, 2);
  snprintf(items[8], sizeof(items[8]), "ExtraThr:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.imuPidTestYawError_x100, 2);
  snprintf(items[9], sizeof(items[9]), "YawErr:%s", value);
  snprintf(items[10], sizeof(items[10]), "Sign:%+d", (activeSettings.imuSign_x100 < 0) ? -1 : 1);
  snprintf(items[11], sizeof(items[11]), "IMU PID Test");
  snprintf(items[12], sizeof(items[12]), "Back");
  const char* ptrs[13];
  for (int i = 0; i < 13; ++i) ptrs[i] = items[i];
  drawScrollableItemList(ptrs, 13, gImuPidMenuIndex, 26);
  u8g2.setFont(u8g2_font_5x8_tf);
...
void drawPIDTestScreen() {
  drawHeader("PID Test");
  PerceptionFrame frame = buildPerceptionFrame(gPidTestHeading);
  PidSource displaySource = pidSourceForTestMode(gPidTestMode);
  ...
  char line[32];
  snprintf(line, sizeof(line), "Dir:%s Mode:%s", headingToString(gPidTestHeading), pidTestModeToString(gPidTestMode));
  u8g2.drawStr(0, 21, line);
  snprintf(line, sizeof(line), "L:%2.1f R:%2.1f", frame.left, frame.right);
  u8g2.drawStr(0, 28, line);
  snprintf(line, sizeof(line), "F:%2.1f Src:%s", frame.front, pidSourceToString(displaySource));
  u8g2.drawStr(0, 35, line);
  snprintf(line, sizeof(line), "Raw:%2.2f Sh:%2.2f", rawError, shaped);
  u8g2.drawStr(0, 42, line);
  snprintf(line, sizeof(line), "Wall:%s IMU:%s",
           wallPidUsageEnabled() ? "ON" : "OFF",
           imuUsageEnabled() ? "ON" : "OFF");
  u8g2.drawStr(0, 49, line);
  u8g2.drawStr(0, 63, "Enc=mode Short=dir Hold=back");
}
...
void drawMotorTuneScreen() {
  ...
  formatCompactValue(value, sizeof(value), activeSettings.servoStopFL_x1000, 3);
  snprintf(items[10], sizeof(items[10]), "FLStop:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.servoStopFR_x1000, 3);
  snprintf(items[11], sizeof(items[11]), "FRStop:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.servoStopRR_x1000, 3);
  snprintf(items[12], sizeof(items[12]), "RRStop:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.servoStopRL_x1000, 3);
...
static void eventConfigFieldText(EventConfigField field, char* buffer, size_t size) {
  ...
  switch (field) {
    ...
    case EventConfigField::OverallSpeed:
      ...
      break;
    case EventConfigField::OverallStart:
      formatCompactValue(value, sizeof(value), activeSettings.overallStart_x100, 2);
      snprintf(buffer, size, "OStart:%s", value);
      break;
    case EventConfigField::OverallRampK:
      formatCompactValue(value, sizeof(value), activeSettings.overallRampK_x100, 2);
      snprintf(buffer, size, "ORamp:%s", value);
      break;
    case EventConfigField::LeftTurnRamp:
      snprintf(buffer, size, "LTurnRamp:%s", activeSettings.leftTurnRampEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::RightTurnRamp:
      snprintf(buffer, size, "RTurnRamp:%s", activeSettings.rightTurnRampEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::TJunctionRamp:
      snprintf(buffer, size, "TJctRamp:%s", activeSettings.tJunctionRampEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::DeadEndRamp:
      snprintf(buffer, size, "DEndRamp:%s", activeSettings.deadEndRampEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::FrontStop:
      formatCompactValue(value, sizeof(value), activeSettings.frontStopDistance_x100, 2);
    ...
    case EventConfigField::FrontHoldRef:
      formatCompactValue(value, sizeof(value), activeSettings.frontHoldReference_x100, 2);
      snprintf(buffer, size, "FrontRef:%s", value);
      break;
    case EventConfigField::FrontPidP:
      formatCompactValue(value, sizeof(value), activeSettings.frontPidP_x1000, 3);
      snprintf(buffer, size, "FrontP:%s", value);
      break;
    case EventConfigField::FrontPidI:
      formatCompactValue(value, sizeof(value), activeSettings.frontPidI_x1000, 3);
      snprintf(buffer, size, "FrontI:%s", value);
      break;
    case EventConfigField::FrontPidD:
      formatCompactValue(value, sizeof(value), activeSettings.frontPidD_x1000, 3);
      snprintf(buffer, size, "FrontD:%s", value);
      break;
    case EventConfigField::TurnYawTol:
      formatCompactValue(value, sizeof(value), activeSettings.turnYawTolerance_x100, 2);
      snprintf(buffer, size, "YawTol:%s", value);
      break;
    case EventConfigField::WallThreshold:
      formatCompactValue(value, sizeof(value), activeSettings.corridorWallThreshold_x100, 2);
    ...
    case EventConfigField::MatchCount:
      ...
      break;
    case EventConfigField::WallToggle:
      snprintf(buffer, size, "WallPID:%s", activeSettings.wallPidEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::OneWallSymmetry:
      snprintf(buffer, size, "1WSym:%s", activeSettings.pid1WallSymmetryEnabled ? "ON" : "OFF");
      break;
    case EventConfigField::WallP:
      formatCompactValue(value, sizeof(value), activeSettings.p_x1000, 3);
      snprintf(buffer, size, "2WP:%s", value);
      break;
    case EventConfigField::WallI:
      formatCompactValue(value, sizeof(value), activeSettings.i_x1000, 3);
      snprintf(buffer, size, "2WI:%s", value);
      break;
    case EventConfigField::WallD:
      formatCompactValue(value, sizeof(value), activeSettings.d_x1000, 3);
      snprintf(buffer, size, "2WD:%s", value);
      break;
    case EventConfigField::CurveB:
      formatCompactValue(value, sizeof(value), activeSettings.curveB_x100, 2);
    ...
    case EventConfigField::OneWallP:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallP_x1000, 3);
      snprintf(buffer, size, "1WP:%s", value);
      break;
    case EventConfigField::OneWallI:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallI_x1000, 3);
      snprintf(buffer, size, "1WI:%s", value);
      break;
    case EventConfigField::OneWallD:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallD_x1000, 3);
      snprintf(buffer, size, "1WD:%s", value);
      break;
    case EventConfigField::OneWallRightP:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightP_x1000, 3);
      snprintf(buffer, size, "1WRP:%s", value);
      break;
    case EventConfigField::OneWallRightI:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightI_x1000, 3);
      snprintf(buffer, size, "1WRI:%s", value);
      break;
    case EventConfigField::OneWallRightD:
      formatCompactValue(value, sizeof(value), activeSettings.pid1WallRightD_x1000, 3);
      snprintf(buffer, size, "1WRD:%s", value);
      break;
    case EventConfigField::OneWallDeadband:
      formatCompactValue(value, sizeof(value), activeSettings.pidDeadband1W_x100, 2);
    ...
    case EventConfigField::ImuLatP:
      formatCompactValue(value, sizeof(value), activeSettings.imuLatP_x10000, 4);
      snprintf(buffer, size, "LatP:%s", value);
      break;
    case EventConfigField::ImuLatI:
      formatCompactValue(value, sizeof(value), activeSettings.imuLatI_x10000, 4);
      snprintf(buffer, size, "LatI:%s", value);
      break;
    case EventConfigField::ImuLatD:
      formatCompactValue(value, sizeof(value), activeSettings.imuLatD_x10000, 4);
      snprintf(buffer, size, "LatD:%s", value);
      break;
    case EventConfigField::ImuLatFilterAlpha:
      formatCompactValue(value, sizeof(value), activeSettings.imuLatFilterAlpha_x100, 2);
      snprintf(buffer, size, "LatA:%s", value);
      break;
    case EventConfigField::ImuThreshold:
      formatCompactValue(value, sizeof(value), activeSettings.imuAngleThreshold_x100, 2);
...
void drawIRSensorTestScreen() {
  drawHeader("IR Test");
  char items[8][28];
  snprintf(items[0], sizeof(items[0]), "NW(FL): %d", isIrTriggered(gIrFL) ? 1 : 0);
  snprintf(items[1], sizeof(items[1]), "NE(FR): %d", isIrTriggered(gIrFR) ? 1 : 0);
  snprintf(items[2], sizeof(items[2]), "SW(RL): %d", isIrTriggered(gIrRL) ? 1 : 0);
  snprintf(items[3], sizeof(items[3]), "SE(RR): %d", isIrTriggered(gIrRR) ? 1 : 0);
  char value[16];
  formatCompactValue(value, sizeof(value), activeSettings.sensingInterval_x100, 2);
...
void drawMazeTestRunScreen() {
  ...
  char line[32];
  snprintf(line, sizeof(line), "State:%s", navStateToString(gPreviewRun.state));
  u8g2.drawStr(0, 20, line);

  snprintf(line, sizeof(line), "Event:%s", eventToString(gPreviewRun.displayEvent));
  u8g2.drawStr(0, 29, line);

  if (startReady) {
    snprintf(line, sizeof(line), "Next:%s Dir:%s",
             routeModeShortString(executionMode),
             headingToString(heading));
  } else {
    snprintf(line, sizeof(line), "%s", errorText ? errorText : "Selection blocked");
  }
  u8g2.drawStr(0, 38, line);
...
void drawStartConfirmScreen() {
  ...
  if (startReady) {
    ...
    snprintf(line, sizeof(line), "%s", errorText ? errorText : "Start blocked");
  }
  u8g2.drawStr(0, 29, line);

  formatPlannerAttemptLine(line, sizeof(line), "R", gPlanner.rightAttempt);
  u8g2.drawStr(0, 38, line);
  formatPlannerAttemptLine(line, sizeof(line), "L", gPlanner.leftAttempt);
  u8g2.drawStr(0, 47, line);
  formatPlannerAttemptLine(line, sizeof(line), "S", gPlanner.selectionAttempt);
  u8g2.drawStr(0, 56, line);
...
void loop() {
  ...
  int encoderDelta = 0;
  while (combinedEncoderDelta >= ENCODER_EDGES_PER_MENU_STEP) {
    encoderDelta++;
    combinedEncoderDelta -= ENCODER_EDGES_PER_MENU_STEP;
  }
  while (combinedEncoderDelta <= -ENCODER_EDGES_PER_MENU_STEP) {
    encoderDelta--;
    combinedEncoderDelta += ENCODER_EDGES_PER_MENU_STEP;
  ...
  if (gExecutive.running) {
    ...
    if (gCurrentScreen == Screen::PIDTest) {
      refreshSensors();
      updatePidTestDrive();
    } else {
      stopAll();
  ...
  } else if (gCurrentScreen == Screen::MazeTestRun) {
    ButtonEvent buttonEvent = readButtonEvent(true);
    handleMazeTestRunInput(encoderDelta, buttonEvent);
    refreshSensors();
    stopAll();
    if (gCurrentScreen == Screen::MazeTestRun) {
      updatePreviewExecution();
    }
  } else {
    ButtonEvent buttonEvent = readButtonEvent(true);
