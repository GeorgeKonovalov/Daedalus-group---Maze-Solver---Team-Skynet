...
static void runTuneMotion() {
  switch (gTuneMotion) {
    ...
    case TuneMotion::North: driveRobotFrame(0.0f, gCfg.baseSpeed, Heading::North); break;
    case TuneMotion::East:  driveRobotFrame(0.0f, gCfg.baseSpeed, Heading::East); break;
    case TuneMotion::South: driveRobotFrame(0.0f, gCfg.baseSpeed, Heading::South); break;
    case TuneMotion::West:  driveRobotFrame(0.0f, gCfg.baseSpeed, Heading::West); break;
  }
}
...
void handleRootInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gRootIndex) {
    ...
    case 4: gPreviewHeading = headingFromIndex(activeSettings.startHeadingIndex); gCurrentScreen = Screen::MazeTestRun; break;
    case 5: gCurrentScreen = Screen::IRSensorTest; break;
    case 6: gCurrentScreen = Screen::UltrasonicSensorTest; break;
    default: break;
  }
}
...
void handlePidInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gPidIndex) {
    ...
    case 8: gCurrentScreen = Screen::Settings; break;
    default: break;
  }
}
...
void drawScrollableItemList(const char* const* items, int itemCount, int selectedIndex, int startY) {
  constexpr int visibleCount = 5;
  int startIndex = 0;
  if (selectedIndex >= visibleCount) startIndex = selectedIndex - visibleCount + 1;
  if (startIndex > itemCount - visibleCount) startIndex = max(0, itemCount - visibleCount);

  int endIndex = min(itemCount, startIndex + visibleCount);
  int row = 0;
  for (int i = startIndex; i < endIndex; ++i) {
    drawMenuItem(startY + row * 9, i == selectedIndex, items[i]);
    row++;
  }
...
void drawRootScreen() {
  ...
  drawScrollableItemList(items, 7, gRootIndex, 26);
}
...
void drawPIDScreen() {
  ...
  drawScrollableItemList(ptrs, 9, gPidIndex, 26);
}
...
void drawEventTestConfigScreen() {
  ...
  u8g2.setFont(u8g2_font_5x8_tr);
  u8g2.drawStr(0, 63, "New verify logic");
}
