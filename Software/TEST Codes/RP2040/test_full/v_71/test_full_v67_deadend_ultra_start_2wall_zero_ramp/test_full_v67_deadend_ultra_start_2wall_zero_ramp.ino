...
static EventType sequentialEventAfterCorrection(const PerceptionFrame& frame) {
  if (!frame.ultrasonicValid) return EventType::SensorWait;

  bool frontBlocked = frame.front < gCfg.corridorWallThresholdCm;
  bool leftOpen = frame.left > gCfg.corridorWallThresholdCm;
  bool rightOpen = frame.right > gCfg.corridorWallThresholdCm;
  bool leftBlocked = !leftOpen;
  bool rightBlocked = !rightOpen;
  ...
  if (frontBlocked && leftOpen && rightBlocked) {
    return EventType::LeftTurn;
  }
  if (frontBlocked && rightOpen && leftBlocked) {
    return EventType::RightTurn;
  }
...
static bool frontNormalizationComplete(const PerceptionFrame& frame, EventType eventType) {
  if (eventType == EventType::Start) {
    ...
    float wallError = computeWallError(frame, PidSource::TwoWall);
    float allowedWallError = max(gCfg.pidDeadband2W, 0.01f);
    if (fabsf(wallError) > allowedWallError) return false;
...
static void runFrontNormalization(Heading heading, const PerceptionFrame& frame, EventType eventType) {
  ...
  PidSource controlSource = (eventType == EventType::Start)
                              ? PidSource::TwoWall
                              : stabilizedPidSource(frame, gActivePidSource);
...
void updateExecutive() {
  ...
  switch (gExecutive.state) {
    ...
    case NavState::FrontNormalize: {
      ...
      EventType nextEvent = (gExecutive.latchedEvent == EventType::DeadEnd)
                              ? EventType::DeadEnd
                              : sequentialEventAfterCorrection(frame);

      if (nextEvent == EventType::SensorWait) {
        stopAll();
        gExecutive.displayEvent = EventType::SensorWait;
        return;
      }

      if (nextEvent == EventType::DeadEnd) {
        gPlanner.currentAttemptHadDeadEnd = true;
        gExecutive.deadEndRaised = true;
      }
      if (nextEvent == EventType::TJunction) {
        gExecutive.finishArmed = true;
        gExecutive.tJunctionRaised = true;
      }

      gExecutive.latchedEvent = nextEvent;
      gExecutive.pendingHeading = chooseHeadingForEvent(frame, nextEvent, gExecutive.heading);
      gExecutive.heading = gExecutive.pendingHeading;
      resetDrivePidControl();
      gExecutive.postCorrectionReleaseActive = true;
      prepareOverallSpeedAfterStopEvent(nextEvent);
      enterPhase(NavState::AcquireCorridor, nextEvent);
      return;
    }
