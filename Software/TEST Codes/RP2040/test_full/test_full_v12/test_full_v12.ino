...
SensorObservation buildSensorObservation(const SensorSnapshot& snapshot, Heading heading, float deadEndThresholdCm,
  ...
  observation.candidateEvent = classifySensorEventWithThresholds(
    snapshot, heading, true, deadEndThresholdCm, turningThresholdCm, wallThresholdCm);
  return observation;
}
...
bool finishFrontClearEnough(float frontCm, float deadEndThresholdCm) {
  return frontCm >= ((deadEndThresholdCm * 2.0f) + EVENT_CONFIRM_MARGIN_CM);
}
...
EventName classifySensorEventWithThresholds(const SensorSnapshot& snapshot, Heading heading, bool allowFinishEvent,
  ...
  if (allowFinishEvent &&
      ...
      frontPairClear &&
      rearPairClear &&
      finishFrontClearEnough(front, deadEndThresholdCm) &&
      leftOpenStable &&
      rightOpenStable) {
