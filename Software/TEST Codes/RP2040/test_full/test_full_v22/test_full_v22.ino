...
float gPidLeftScale = 1.0f;
float gPidRightScale = 1.0f;
...
int startSettingsIndex = 0; // 0=Route mode, 1=Turn wait, 2=Back
int settingsIndex = 0;      // 0=PID, 1=Motor Tune, 2=Front Stop, 3=Corridor Wall, 4=Turn Detect, 5=Recheck, 6=Save
int pidIndex = 0;           // 0=P, 1=I, 2=D, 3=Curve b, 4=1-wall target, 5=Left scale, 6=Right scale, 7=PID test, 8=Back
...
struct DirectionMagnitude {
  int8_t direction = 0;   // -1 = left, +1 = right
...
float pidPreviewRawMagnitude = 0.0f;
float pidPreviewInterpretedError = 0.0f;
float pidPreviewInterpretedMagnitude = 0.0f;
float pidPreviewCorrection = 0.0f;
float pidPreviewScaledCorrection = 0.0f;
int8_t pidPreviewDirection = 0;
...
const float CORRECTION_SCALE = 1.40f;
...
constexpr int EVENT_TEST_CONFIG_MAX_COUNT = 20;
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_CORRIDOR[] = {
  EventConfigField::Heading,
  EventConfigField::CorridorWallDistance,
  EventConfigField::PGain,
  EventConfigField::IGain,
  EventConfigField::DGain,
  EventConfigField::CurveExponent,
  EventConfigField::OneWallTarget,
  EventConfigField::LeftOutputScale,
  EventConfigField::RightOutputScale,
  EventConfigField::Run,
  EventConfigField::Back
};

constexpr EventConfigField EVENT_CONFIG_FIELDS_T_JUNCTION[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
  EventConfigField::TurnChoice,
  EventConfigField::TurnWaitTime,
  EventConfigField::RecheckTime,
  EventConfigField::IRStableTime,
  EventConfigField::DeadEndDistance,
  EventConfigField::TurnDetectDistance,
  EventConfigField::PGain,
  EventConfigField::IGain,
  EventConfigField::DGain,
  EventConfigField::CurveExponent,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_DEAD_END[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_START[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_FINISH[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
  EventConfigField::FinishActionTime,
  EventConfigField::RecheckTime,
  EventConfigField::IRStableTime,
  EventConfigField::DeadEndDistance,
  EventConfigField::TurnDetectDistance,
  EventConfigField::FinishDetectDistance,
  EventConfigField::PGain,
  EventConfigField::IGain,
  EventConfigField::DGain,
  EventConfigField::CurveExponent,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_LEFT_TURN[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_RIGHT_TURN[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_T_STRAIGHT[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
constexpr EventConfigField EVENT_CONFIG_FIELDS_RANDOM[] = {
  EventConfigField::Heading,
  EventConfigField::EventStopDistance,
  EventConfigField::EventWallDistance,
  EventConfigField::CorridorWallDistance,
...
int8_t directionFromSignedValue(float value);
float interpretMagnitude(float magnitude);
float signedValueFromDirectionAndMagnitude(int8_t direction, float magnitude);
DirectionMagnitude buildDirectionalMagnitude(float signedValue);
...
EventName eventTestCaseToSensorEvent(EventTestCase testCase);
const char* turnChoiceToString(TurnChoice choice);
EventTestConfig& currentEventTestConfig();
const EventConfigField* currentEventConfigFields();
int currentEventConfigItemCount();
EventConfigField currentEventConfigFieldAt(int index);
String eventConfigFieldText(EventConfigField field);
...
const char* eventConfigFieldDescriptionLine1(EventConfigField field);
const char* eventConfigFieldDescriptionLine2(EventConfigField field);
...
void openDigitEditor(const char* label, int* target, int minValue, int maxValue,
                     const char* descriptionLine1 = "", const char* descriptionLine2 = "");
...
float interpretMagnitude(float magnitude) {
  ...
  if (magnitude < 0.6f) {
    ...
    return 0.6f * pow(magnitude / 0.6f, gPidCurveB);
  }
  return magnitude;
}
...
DirectionMagnitude buildDirectionalMagnitude(float signedValue) {
  DirectionMagnitude result;
  result.direction = directionFromSignedValue(signedValue);
  result.rawMagnitude = fabs(signedValue);
  result.interpretedMagnitude = interpretMagnitude(result.rawMagnitude);
  result.signedValue = signedValueFromDirectionAndMagnitude(result.direction, result.interpretedMagnitude);
  return result;
}

float interpretValue(float x) {
  DirectionMagnitude directionalValue = buildDirectionalMagnitude(x);
  return directionalValue.signedValue;
}
...
float rawWallPidErrorForHeading(Heading heading, float wallThresholdCm, bool& pidActive) {
  ...
  if (leftWallPresent && rightWallPresent) {
    pidActive = true;
    return corridorRawErrorForHeading(heading);
  }

  if (leftWallPresent) {
    pidActive = true;
    return gPidWallDistanceCm - averagedDistanceForHeading(turnLeftOf(heading));
  }

  if (rightWallPresent) {
    pidActive = true;
    return averagedDistanceForHeading(turnRightOf(heading)) - gPidWallDistanceCm;
  }
...
float applyDirectionalCorrectionScale(float correction) {
  ...
  if (correction < 0.0f) {
    return constrain(correction * gPidLeftScale, -CORRECTION_LIMIT, CORRECTION_LIMIT);
  }
...
void resetPIDPreview() {
  ...
  pidPreviewScaledCorrection = 0.0f;
  pidPreviewDirection = 0;
  pidPreviewRawError = 0.0f;
  pidPreviewRawMagnitude = 0.0f;
  pidPreviewInterpretedError = 0.0f;
  pidPreviewInterpretedMagnitude = 0.0f;
...
void updatePIDPreview() {
  ...
  pidPreviewRawError = rawWallPidErrorForHeading(pidTestHeading, gWallThresholdCm, pidActive);

  DirectionMagnitude directionalError = buildDirectionalMagnitude(pidPreviewRawError);
  pidPreviewDirection = directionalError.direction;
  pidPreviewRawMagnitude = directionalError.rawMagnitude;
  pidPreviewInterpretedMagnitude = directionalError.interpretedMagnitude;
  pidPreviewInterpretedError = directionalError.signedValue;
  ...
  pidPreviewScaledCorrection = applyDirectionalCorrectionScale(pidPreviewCorrection);
...
bool isIRTriggered(int value) {
  // This robot now treats a HIGH/1 IR input as "wall detected".
  return value == HIGH;
}
...
RobotTwinCorners robotTwinCornersForMask(uint8_t irMask, Heading heading) {
  bool northWest = (irMask & 0x04) != 0;
  bool northEast = (irMask & 0x08) != 0;
  bool southWest = (irMask & 0x01) != 0;
  bool southEast = (irMask & 0x02) != 0;

  RobotTwinCorners corners;
  switch (heading) {
    case Heading::North:
      corners.frontLeft = northWest;
      corners.frontRight = northEast;
      corners.rearLeft = southWest;
      corners.rearRight = southEast;
      break;
    case Heading::East:
      corners.frontLeft = northEast;
      corners.frontRight = southEast;
      corners.rearLeft = northWest;
      corners.rearRight = southWest;
      break;
    case Heading::South:
      corners.frontLeft = southEast;
      corners.frontRight = southWest;
      corners.rearLeft = northEast;
      corners.rearRight = northWest;
      break;
    case Heading::West:
      corners.frontLeft = southWest;
      corners.frontRight = northWest;
      corners.rearLeft = southEast;
      corners.rearRight = northEast;
      break;
  }
  return corners;
}
...
void commitGlobalsForRuntime() {
  ...
  gPidLeftScale = activeSettings.pidLeftScale_x100 / 100.0f;
  gPidRightScale = activeSettings.pidRightScale_x100 / 100.0f;
...
EventName eventTestCaseToSensorEvent(EventTestCase testCase) {
  switch (testCase) {
    ...
    case EventTestCase::Start:             return EventName::Idle;
...
const EventConfigField* currentEventConfigFields() {
  switch (selectedEventTest) {
    case EventTestCase::Corridor:          return EVENT_CONFIG_FIELDS_CORRIDOR;
    case EventTestCase::TJunction:         return EVENT_CONFIG_FIELDS_T_JUNCTION;
    case EventTestCase::DeadEnd:           return EVENT_CONFIG_FIELDS_DEAD_END;
    case EventTestCase::Start:             return EVENT_CONFIG_FIELDS_START;
    case EventTestCase::Finish:            return EVENT_CONFIG_FIELDS_FINISH;
    case EventTestCase::LeftTurn:          return EVENT_CONFIG_FIELDS_LEFT_TURN;
    case EventTestCase::RightTurn:         return EVENT_CONFIG_FIELDS_RIGHT_TURN;
    case EventTestCase::TJunctionStraight: return EVENT_CONFIG_FIELDS_T_STRAIGHT;
    case EventTestCase::Random:            return EVENT_CONFIG_FIELDS_RANDOM;
    case EventTestCase::Back:
    default:                               return EVENT_CONFIG_FIELDS_CORRIDOR;
  }
}

int currentEventConfigItemCount() {
  switch (selectedEventTest) {
    case EventTestCase::Corridor:          return sizeof(EVENT_CONFIG_FIELDS_CORRIDOR) / sizeof(EVENT_CONFIG_FIELDS_CORRIDOR[0]);
    case EventTestCase::TJunction:         return sizeof(EVENT_CONFIG_FIELDS_T_JUNCTION) / sizeof(EVENT_CONFIG_FIELDS_T_JUNCTION[0]);
    case EventTestCase::DeadEnd:           return sizeof(EVENT_CONFIG_FIELDS_DEAD_END) / sizeof(EVENT_CONFIG_FIELDS_DEAD_END[0]);
    case EventTestCase::Start:             return sizeof(EVENT_CONFIG_FIELDS_START) / sizeof(EVENT_CONFIG_FIELDS_START[0]);
    case EventTestCase::Finish:            return sizeof(EVENT_CONFIG_FIELDS_FINISH) / sizeof(EVENT_CONFIG_FIELDS_FINISH[0]);
    case EventTestCase::LeftTurn:          return sizeof(EVENT_CONFIG_FIELDS_LEFT_TURN) / sizeof(EVENT_CONFIG_FIELDS_LEFT_TURN[0]);
    case EventTestCase::RightTurn:         return sizeof(EVENT_CONFIG_FIELDS_RIGHT_TURN) / sizeof(EVENT_CONFIG_FIELDS_RIGHT_TURN[0]);
    case EventTestCase::TJunctionStraight: return sizeof(EVENT_CONFIG_FIELDS_T_STRAIGHT) / sizeof(EVENT_CONFIG_FIELDS_T_STRAIGHT[0]);
    case EventTestCase::Random:            return sizeof(EVENT_CONFIG_FIELDS_RANDOM) / sizeof(EVENT_CONFIG_FIELDS_RANDOM[0]);
    case EventTestCase::Back:
    default:                               return sizeof(EVENT_CONFIG_FIELDS_CORRIDOR) / sizeof(EVENT_CONFIG_FIELDS_CORRIDOR[0]);
  }
}

EventConfigField currentEventConfigFieldAt(int index) {
  int itemCount = currentEventConfigItemCount();
  if (itemCount <= 0) return EventConfigField::Back;
  index = clampInt(index, 0, itemCount - 1);
  return currentEventConfigFields()[index];
}
...
String eventConfigFieldText(EventConfigField field) {
  ...
  switch (field) {
    case EventConfigField::Heading:
      return "Heading: " + String(headingToString(headingFromIndex(cfg.heading)));
    case EventConfigField::EventStopDistance:
      return "FrontStop(cm): " + formatX100(cfg.stopThreshold_x100);
    case EventConfigField::EventWallDistance:
      return "EventWall(cm): " + formatX100(cfg.sideThreshold_x100);
    case EventConfigField::CorridorWallDistance:
      return "CorrWall(cm): " + formatX100(workingSettings.wallThreshold_x100);
    case EventConfigField::FinishActionTime:
      return "FinishTime(s): " + formatX100(cfg.actionTime_x100);
    case EventConfigField::TurnChoice:
      return "T-Dir: " + String(turnChoiceToString(cfg.turnChoice));
    case EventConfigField::TurnWaitTime:
      return "TurnWait(s): " + formatX100(workingSettings.waitBeforeTurn_x100);
    case EventConfigField::RecheckTime:
      return "Recheck(s): " + formatX100(workingSettings.sensingInterval_x100);
    case EventConfigField::IRStableTime:
      return "IRStable(s): " + formatX100(workingSettings.irStableTime_x100);
    case EventConfigField::DeadEndDistance:
      return "DeadEnd(cm): " + formatX100(workingSettings.deadEndDistance_x100);
    case EventConfigField::TurnDetectDistance:
      return "TurnDet(cm): " + formatX100(workingSettings.turningThreshold_x100);
    case EventConfigField::FinishDetectDistance:
      return "Finish(cm): " + formatX100(workingSettings.finishDistance_x100);
    case EventConfigField::PGain:
      return "P Gain: " + formatX100(workingSettings.p_x100);
    case EventConfigField::IGain:
      return "I Gain: " + formatX100(workingSettings.i_x100);
    case EventConfigField::DGain:
      return "D Gain: " + formatX100(workingSettings.d_x100);
    case EventConfigField::CurveExponent:
      return "Curve b: " + formatX100(workingSettings.pidCurveB_x100);
    case EventConfigField::OneWallTarget:
      return "1Wall(cm): " + formatX100(workingSettings.pidWallDistance_x100);
    case EventConfigField::LeftOutputScale:
      return "LeftOut(x): " + formatX100(workingSettings.pidLeftScale_x100);
    case EventConfigField::RightOutputScale:
      return "RightOut(x): " + formatX100(workingSettings.pidRightScale_x100);
...
const char* eventConfigFieldDescriptionLine1(EventConfigField field) {
  switch (field) {
    case EventConfigField::Heading:               return "Robot heading used";
    case EventConfigField::EventStopDistance:     return "Front gap to stop";
    case EventConfigField::EventWallDistance:     return "Wall lock used in";
    case EventConfigField::CorridorWallDistance:  return "Wall seen for PID";
    case EventConfigField::FinishActionTime:      return "Extra straight time";
    case EventConfigField::TurnChoice:            return "Used only for the";
    case EventConfigField::TurnWaitTime:          return "Pause after stop";
    case EventConfigField::RecheckTime:           return "Gap between event";
    case EventConfigField::IRStableTime:          return "Sensors must hold";
    case EventConfigField::DeadEndDistance:       return "Front <= this is";
    case EventConfigField::TurnDetectDistance:    return "Front <= this can";
    case EventConfigField::FinishDetectDistance:  return "Front >= this can";
    case EventConfigField::PGain:                 return "Main steering gain";
    case EventConfigField::IGain:                 return "Bias correction";
    case EventConfigField::DGain:                 return "Fast change gain";
    case EventConfigField::CurveExponent:         return "Shapes small wall";
    case EventConfigField::OneWallTarget:         return "Target when only";
    case EventConfigField::LeftOutputScale:       return "Extra scale when";
    case EventConfigField::RightOutputScale:      return "Extra scale when";
...
const char* eventConfigFieldDescriptionLine2(EventConfigField field) {
  switch (field) {
    case EventConfigField::Heading:               return "before the test.";
    case EventConfigField::EventStopDistance:     return "before turn logic.";
    case EventConfigField::EventWallDistance:     return "event handling [cm].";
    case EventConfigField::CorridorWallDistance:  return "inside corridor [cm].";
    case EventConfigField::FinishActionTime:      return "after finish [s].";
    case EventConfigField::TurnChoice:            return "T-junction branch.";
    case EventConfigField::TurnWaitTime:          return "before heading swap.";
    case EventConfigField::RecheckTime:           return "confirm samples [s].";
    case EventConfigField::IRStableTime:          return "steady before use [s].";
    case EventConfigField::DeadEndDistance:       return "dead-end zone [cm].";
    case EventConfigField::TurnDetectDistance:    return "count as turn [cm].";
    case EventConfigField::FinishDetectDistance:  return "count as finish [cm].";
    case EventConfigField::PGain:                 return "from wall error.";
    case EventConfigField::IGain:                 return "for steady drift.";
    case EventConfigField::DGain:                 return "for sharp changes.";
    case EventConfigField::CurveExponent:         return "errors before PID.";
    case EventConfigField::OneWallTarget:         return "one wall exists [cm].";
    case EventConfigField::LeftOutputScale:       return "command goes left.";
    case EventConfigField::RightOutputScale:      return "command goes right.";
    case EventConfigField::Run:                   return "event scenario.";
    case EventConfigField::Back:
...
void startEventTestRun() {
  ...
  eventTestResolvedEvent = (selectedEventTest == EventTestCase::Start)
    ? EventName::Start
    : eventTestCaseToSensorEvent(selectedEventTest);
...
void openDigitEditor(const char* label, int* target, int minValue, int maxValue,
  ...
  digitEditor.descriptionLine1 = descriptionLine1;
  digitEditor.descriptionLine2 = descriptionLine2;
...
void closeDigitEditor(bool commit) {
  ...
  digitEditor.digitUnlocked = false;
  digitEditor.descriptionLine1 = "";
  digitEditor.descriptionLine2 = "";
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (startSettingsIndex) {
      case 0:
        ...
        break;
      case 1:
        openDigitEditor("Turn Wait(s)", &workingSettings.waitBeforeTurn_x100, 0, 9999,
                        "Pause after front stop", "before heading change.");
        break;
      case 2:
        currentScreen = Screen::Root;
        break;
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (settingsIndex) {
      ...
      case 1:
        ...
        break;
      case 2:
        openDigitEditor("Front Stop(cm)", &workingSettings.junctionStopDistance_x100, 0, 9999,
                        "Front gap to stop", "before turn logic.");
        break;
      case 3:
        openDigitEditor("Corridor Wall(cm)", &workingSettings.wallThreshold_x100, 0, 9999,
                        "Wall seen by corridor", "PID selection [cm].");
        break;
      case 4:
        openDigitEditor("Turn Detect(cm)", &workingSettings.turningThreshold_x100, 0, 9999,
                        "Front <= this still", "counts as turn [cm].");
        break;
      case 5:
        openDigitEditor("Recheck Time(s)", &workingSettings.sensingInterval_x100, 1, 9999,
                        "Gap between repeated", "event checks [s].");
        break;
      case 6:
        commitGlobalsForRuntime();
        currentScreen = Screen::Root;
...
void handlePIDInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (pidIndex) {
      case 0: openDigitEditor("P Gain", &workingSettings.p_x100, 0, 9999,
                              "Main steering gain", "from wall error."); break;
      case 1: openDigitEditor("I Gain", &workingSettings.i_x100, 0, 9999,
                              "Bias correction", "for steady drift."); break;
      case 2: openDigitEditor("D Gain", &workingSettings.d_x100, 0, 9999,
                              "Fast change gain", "for sharp changes."); break;
      case 3: openDigitEditor("Curve b", &workingSettings.pidCurveB_x100, 1, 9999,
                              "Shapes small wall", "errors before PID."); break;
      case 4: openDigitEditor("1Wall Target(cm)", &workingSettings.pidWallDistance_x100, 0, 9999,
                              "Target when only", "one wall exists [cm]."); break;
      case 5: openDigitEditor("Left Scale(x)", &workingSettings.pidLeftScale_x100, 10, 300,
                              "Extra scale when", "command goes left."); break;
      case 6: openDigitEditor("Right Scale(x)", &workingSettings.pidRightScale_x100, 10, 300,
                              "Extra scale when", "command goes right."); break;
      case 7:
        pidTestHeading = Heading::North;
        resetPIDPreview();
        currentScreen = Screen::PIDTest;
        break;
      case 8: currentScreen = Screen::Settings; break;
    }
  }
...
void handleMotorTuneInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (motorTuneIndex) {
      ...
      case 1:  openDigitEditor("PairA Fwd(x)", &workingSettings.pairFLRR_fwd_x100, 10, 300,
                               "Scale for FL + RR", "while moving forward."); break;
      case 2:  openDigitEditor("PairA Rev(x)", &workingSettings.pairFLRR_back_x100, 10, 300,
                               "Scale for FL + RR", "while moving reverse."); break;
      case 3:  openDigitEditor("PairB Fwd(x)", &workingSettings.pairFRRL_fwd_x100, 10, 300,
                               "Scale for FR + RL", "while moving forward."); break;
      case 4:  openDigitEditor("PairB Rev(x)", &workingSettings.pairFRRL_back_x100, 10, 300,
                               "Scale for FR + RL", "while moving reverse."); break;

      case 5:  openDigitEditor("FL Fwd(x)", &workingSettings.fl_fwd_x100, 10, 300,
                               "Front-left wheel gain", "in forward motion."); break;
      case 6:  openDigitEditor("FL Rev(x)", &workingSettings.fl_back_x100, 10, 300,
                               "Front-left wheel gain", "in reverse motion."); break;
      case 7:  openDigitEditor("FR Fwd(x)", &workingSettings.fr_fwd_x100, 10, 300,
                               "Front-right wheel gain", "in forward motion."); break;
      case 8:  openDigitEditor("FR Rev(x)", &workingSettings.fr_back_x100, 10, 300,
                               "Front-right wheel gain", "in reverse motion."); break;
      case 9:  openDigitEditor("RR Fwd(x)", &workingSettings.rr_fwd_x100, 10, 300,
                               "Rear-right wheel gain", "in forward motion."); break;
      case 10: openDigitEditor("RR Rev(x)", &workingSettings.rr_back_x100, 10, 300,
                               "Rear-right wheel gain", "in reverse motion."); break;
      case 11: openDigitEditor("RL Fwd(x)", &workingSettings.rl_fwd_x100, 10, 300,
                               "Rear-left wheel gain", "in forward motion."); break;
      case 12: openDigitEditor("RL Rev(x)", &workingSettings.rl_back_x100, 10, 300,
                               "Rear-left wheel gain", "in reverse motion."); break;
...
void handleEventTestConfigInput(int delta, ButtonEvent btn) {
  ...
  if (delta > 0) {
    ...
    if (eventTestConfigIndex >= itemCount) eventTestConfigIndex = itemCount - 1;
  } else if (delta < 0) {
    eventTestConfigIndex--;
  ...
  if (btn == ButtonEvent::ShortPress) {
    EventConfigField field = currentEventConfigFieldAt(eventTestConfigIndex);
    ...
    switch (field) {
      case EventConfigField::Heading:
        ...
        break;
      case EventConfigField::EventStopDistance:
        openDigitEditor("Front Stop(cm)", &cfg.stopThreshold_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::EventWallDistance:
        openDigitEditor("Event Wall(cm)", &cfg.sideThreshold_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::CorridorWallDistance:
        openDigitEditor("Corridor Wall(cm)", &workingSettings.wallThreshold_x100, 0, 9999,
        ...
        break;
      case EventConfigField::FinishActionTime:
        openDigitEditor("Finish Time(s)", &cfg.actionTime_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::TurnChoice:
      ...
      case EventConfigField::TurnDetectDistance:
        ...
        break;
      case EventConfigField::FinishDetectDistance:
        openDigitEditor("Finish Detect(cm)", &workingSettings.finishDistance_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::PGain:
        openDigitEditor("P Gain", &workingSettings.p_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::IGain:
        openDigitEditor("I Gain", &workingSettings.i_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::DGain:
        openDigitEditor("D Gain", &workingSettings.d_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::CurveExponent:
        openDigitEditor("Curve b", &workingSettings.pidCurveB_x100, 1, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::OneWallTarget:
        openDigitEditor("1Wall Target(cm)", &workingSettings.pidWallDistance_x100, 0, 9999,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::LeftOutputScale:
        openDigitEditor("Left Scale(x)", &workingSettings.pidLeftScale_x100, 10, 300,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::RightOutputScale:
        openDigitEditor("Right Scale(x)", &workingSettings.pidRightScale_x100, 10, 300,
                        eventConfigFieldDescriptionLine1(field), eventConfigFieldDescriptionLine2(field));
        break;
      case EventConfigField::Run:
        startEventTestRun();
...
void handleIRSensorTestInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (irSensorIndex) {
      case 2:
        openDigitEditor("IR Stable(s)", &workingSettings.irStableTime_x100, 1, 9999,
                        "Sensors must hold", "steady before use [s].");
        break;
      case 3:
        openDigitEditor("Dead-End(cm)", &workingSettings.deadEndDistance_x100, 0, 9999,
                        "Front <= this is", "dead-end zone [cm].");
        break;
      case 4:
        openDigitEditor("Finish Detect(cm)", &workingSettings.finishDistance_x100, 0, 9999,
                        "Front >= this can", "count as finish [cm].");
        break;
      case 5:
        currentScreen = Screen::Root;
        break;
...
void drawStartSettingsScreen() {
  ...
  items[0] = "Route Mode: " + String(modeToString(workingSettings.mode));
  items[1] = "Turn Wait(s): " + formatX100(workingSettings.waitBeforeTurn_x100);
  items[2] = "Back";
...
void drawSettingsScreen() {
  ...
  items[0] = "PID Control";
  items[1] = "Motor Calibration";
  items[2] = "FrontStop(cm): " + formatX100(workingSettings.junctionStopDistance_x100);
  items[3] = "CorrWall(cm): " + formatX100(workingSettings.wallThreshold_x100);
  items[4] = "TurnDet(cm): " + formatX100(workingSettings.turningThreshold_x100);
  items[5] = "Recheck(s): " + formatX100(workingSettings.sensingInterval_x100);
  items[6] = "Save & Back";
...
void drawPIDScreen() {
  ...
  items[0] = "P Gain: " + formatX100(workingSettings.p_x100);
  items[1] = "I Gain: " + formatX100(workingSettings.i_x100);
  items[2] = "D Gain: " + formatX100(workingSettings.d_x100);
  items[3] = "Curve b: " + formatX100(workingSettings.pidCurveB_x100);
  items[4] = "1Wall(cm): " + formatX100(workingSettings.pidWallDistance_x100);
  items[5] = "LeftOut(x): " + formatX100(workingSettings.pidLeftScale_x100);
  items[6] = "RightOut(x): " + formatX100(workingSettings.pidRightScale_x100);
  items[7] = "PID Live Test";
  items[8] = "Back";
...
void drawPIDTestScreen() {
  ...
  String line1 = String("Dir:") + headingToString(pidTestHeading) +
                 " WD:" + formatX100(workingSettings.pidWallDistance_x100);
  String line2 = String("b:") + formatX100(workingSettings.pidCurveB_x100) +
                 " G:" + formatX100(workingSettings.pidLeftScale_x100) +
                 "/" + formatX100(workingSettings.pidRightScale_x100);
  ...
  String line4 = String("Raw:") + formatSignedFloat(pidPreviewRawError, 2) +
                 " D:" + (pidPreviewDirection > 0 ? "R" : (pidPreviewDirection < 0 ? "L" : "0"));
  String line5 = String("Mag:") + formatSignedFloat(pidPreviewRawMagnitude, 2) +
                 " In:" + formatSignedFloat(pidPreviewInterpretedMagnitude, 2);
  String line6 = String("PID:") + formatSignedFloat(pidPreviewCorrection, 2) +
                 " Out:" + formatSignedFloat(pidPreviewScaledCorrection, 2);
...
void drawEventTestConfigScreen() {
  drawHeader(eventTestCaseToString(selectedEventTest));

  int itemCount = currentEventConfigItemCount();
  String items[EVENT_TEST_CONFIG_MAX_COUNT];
  for (int i = 0; i < itemCount; ++i) {
    items[i] = eventConfigFieldText(currentEventConfigFieldAt(i));
  }
  drawScrollableItemList(items, itemCount, eventTestConfigIndex, 26);
...
RobotTwinCorners robotTwinCornersForHeading(Heading heading) {
  return robotTwinCornersForMask(buildIRMask(), heading);
}

void drawRobotTwinCorner(int x, int y, bool triggered) {
  if (triggered) {
    u8g2.drawDisc(x, y, 2);
  } else {
...
void drawCenteredEventLabel(const char* text) {
...
void drawMazeTestRunScreen() {
  ...
  const int bodyW = 30;
  const int bodyH = 20;
  const int bodyRight = bodyX + bodyW - 1;
  const int bodyBottom = bodyY + bodyH - 1;

  RobotTwinCorners corners = robotTwinCornersForHeading(mazeTestHeading);
  bool frontPairTriggered = corners.frontLeft && corners.frontRight;
  bool rightPairTriggered = corners.frontRight && corners.rearRight;
  bool backPairTriggered = corners.rearLeft && corners.rearRight;
  bool leftPairTriggered = corners.frontLeft && corners.rearLeft;

  bool ultrasonicValid = ultrasonicFresh();
  bool frontWallVisible = ultrasonicValid && distanceWithinWallThreshold(frontDistanceForHeading(mazeTestHeading), gTurningThresholdCm);
  bool rightWallVisible = ultrasonicValid && distanceWithinWallThreshold(rightDistanceForHeading(mazeTestHeading), gWallThresholdCm);
  bool backWallVisible = ultrasonicValid && distanceWithinWallThreshold(averagedDistanceForHeading(oppositeOf(mazeTestHeading)), gTurningThresholdCm);
  bool leftWallVisible = ultrasonicValid && distanceWithinWallThreshold(leftDistanceForHeading(mazeTestHeading), gWallThresholdCm);

  u8g2.setFont(u8g2_font_6x10_tf);
  drawCenteredEventLabel(eventNameToString(mazeTestEvent));

  // Near-wall layout from ultrasonic readings.
  if (frontWallVisible) u8g2.drawHLine(bodyX + 5, bodyY - 5, bodyW - 10);
  if (backWallVisible)  u8g2.drawHLine(bodyX + 5, bodyBottom + 6, bodyW - 10);
  if (leftWallVisible)  u8g2.drawVLine(bodyX - 10, bodyY + 3, bodyH - 6);
  if (rightWallVisible) u8g2.drawVLine(bodyRight + 10, bodyY + 3, bodyH - 6);

  // Second wall pair slightly away from the rover, driven by the coherent IR side pairs.
  if (frontPairTriggered) {
    u8g2.drawVLine(bodyX - 5, bodyY - 6, 10);
    u8g2.drawVLine(bodyRight + 5, bodyY - 6, 10);
  }
  if (backPairTriggered) {
    u8g2.drawVLine(bodyX - 5, bodyBottom - 3, 10);
    u8g2.drawVLine(bodyRight + 5, bodyBottom - 3, 10);
  }
  if (leftPairTriggered) {
    u8g2.drawHLine(bodyX - 18, bodyY + 5, 10);
    u8g2.drawHLine(bodyX - 18, bodyBottom - 5, 10);
  }
  if (rightPairTriggered) {
    u8g2.drawHLine(bodyRight + 9, bodyY + 5, 10);
    u8g2.drawHLine(bodyRight + 9, bodyBottom - 5, 10);
  }

  // Robot body.
  u8g2.drawFrame(bodyX, bodyY, bodyW, bodyH);
  drawRobotTwinCorner(bodyX + 1, bodyY + 1, corners.frontLeft);
  drawRobotTwinCorner(bodyRight - 1, bodyY + 1, corners.frontRight);
  drawRobotTwinCorner(bodyX + 1, bodyBottom - 1, corners.rearLeft);
  drawRobotTwinCorner(bodyRight - 1, bodyBottom - 1, corners.rearRight);

  // Heading labels inside the robot, rotated with the current execution heading.
  u8g2.setFont(u8g2_font_5x7_tf);
  u8g2.drawStr(bodyX + (bodyW / 2) - 2, bodyY + 8, headingToString(mazeTestHeading));
  u8g2.drawStr(bodyX + 5, bodyY + (bodyH / 2) + 2, headingToString(turnLeftOf(mazeTestHeading)));
  u8g2.drawStr(bodyRight - 8, bodyY + (bodyH / 2) + 2, headingToString(turnRightOf(mazeTestHeading)));
  u8g2.drawStr(bodyX + (bodyW / 2) - 2, bodyBottom - 3, headingToString(oppositeOf(mazeTestHeading)));

  // Bottom ultrasonic readout in the requested 2-line format.
  String line1 = String("N:") + formatDistanceCm(avgDistN) + " S:" + formatDistanceCm(avgDistS);
  String line2 = String("W:") + formatDistanceCm(avgDistW) + " E:" + formatDistanceCm(avgDistE);
  u8g2.drawStr(0, 56, line1.c_str());
  u8g2.drawStr(0, 63, line2.c_str());
}
...
void drawIRSensorTestScreen() {
  ...
  items[2] = "IRStable(s): " + formatX100(workingSettings.irStableTime_x100);
  items[3] = "DeadEnd(cm): " + formatX100(workingSettings.deadEndDistance_x100);
  items[4] = "Finish(cm): " + formatX100(workingSettings.finishDistance_x100);
  items[5] = "Back";
...
void drawDigitEditorScreen() {
  ...
  u8g2.setFont(u8g2_font_5x7_tf);
  if (digitEditor.descriptionLine1 != nullptr && digitEditor.descriptionLine1[0] != '\0') {
    u8g2.drawStr(0, 49, digitEditor.descriptionLine1);
  }
  if (digitEditor.descriptionLine2 != nullptr && digitEditor.descriptionLine2[0] != '\0') {
    u8g2.drawStr(0, 56, digitEditor.descriptionLine2);
  }
  u8g2.drawStr(0, 63, !digitEditor.digitUnlocked ? "Rot=pick Btn=edit" : "Rot=edit Btn=lock");
}
...
void drive(float Vx, float Vy) {
  ...
  float minBase = min(min(fabs(baseFL), fabs(baseFR)),
                      min(fabs(baseRR), fabs(baseRL)));
  float ratio = (minBase > 0.001f) ? (Vx / minBase) : 0.0f;
  ratio *= CORRECTION_SCALE;

  float fl = baseFL * (1.0f + ratio);
  float rr = baseRR * (1.0f + ratio);
  float fr = baseFR * (1.0f - ratio);
  float rl = baseRL * (1.0f - ratio);
...
void followWallAwareHeading(float forwardSpeed, float wallThresholdCm, Heading heading) {
  ...
  if (pidActive) {
    float correction = calculatePID(directionalError.signedValue);
    correction = applyDirectionalCorrectionScale(correction);
...
void setup() {
  ...
  activeSettings.pidLeftScale_x100 = 100;
  activeSettings.pidRightScale_x100 = 100;
  ...
  activeSettings.fr_fwd_x100 = 100;
  activeSettings.fr_back_x100 = 100;
  activeSettings.rr_fwd_x100 = 100;
  activeSettings.rr_back_x100 = 100;
  activeSettings.rl_fwd_x100 = 100;
  activeSettings.rl_back_x100 = 100;
