...
struct RuntimeConfig {
  ...
  float i = 0.0f;
  float d = 0.0f;
  float pid1WallP = 0.0f;
  float pid1WallI = 0.0f;
  float pid1WallD = 0.0f;
  float pid1WallRightP = 0.0f;
  float pid1WallRightI = 0.0f;
  float pid1WallRightD = 0.0f;
...
static bool frontNormalizationComplete(const PerceptionFrame& frame);
static void runFrontNormalization(Heading heading, const PerceptionFrame& frame);
...
static void runFrontNormalization(Heading heading, const PerceptionFrame& frame) {
  ...
  float forwardCorrection = computeFrontPidCorrection(frame.front);
...
void updatePreviewExecution() {
  ...
  switch (gPreviewRun.state) {
    ...
    case NavState::Corridor: {
      ...
      if (eventTriggersEnabledForState(gPreviewRun.state) &&
        ...
        if (confirmed == EventType::DeadEnd) {
          ...
          gPreviewHeading = gPreviewRun.heading;
          startPreviewPhase(NavState::AcquireCorridor, EventType::DeadEnd);
          return;
        }
    ...
    case NavState::FrontNormalize:
      gPreviewRun.displayEvent = gPreviewRun.latchedEvent;
      if (frontNormalizationComplete(frame)) {
        gPreviewRun.heading = gPreviewRun.pendingHeading;
        gPreviewHeading = gPreviewRun.heading;
...
void updateExecutive() {
  ...
  switch (gExecutive.state) {
    ...
    case NavState::FrontNormalize:
      ...
      runFrontNormalization(gExecutive.heading, frame);
      if (frontNormalizationComplete(frame)) {
        gExecutive.heading = gExecutive.pendingHeading;
        resetDrivePidControl();
...
void handleSettingsInput(int encoderDelta, ButtonEvent buttonEvent) {
  ...
  switch (gSettingsIndex) {
    ...
    case 4: openDigitEditor("Front stop", "cm", &workingSettings.frontStopDistance_x100, 0, 9999, "Front distance that", "starts hold stage", Screen::Settings, 2, 2); break;
    case 5: openDigitEditor("Front hold", "cm", &workingSettings.frontHoldReference_x100, 0, 9999, "Front distance held", "before heading swap", Screen::Settings, 2, 2); break;
    case 6: openDigitEditor("Front P", "x", &workingSettings.frontPidP_x1000, 0, 9999, "Front-hold proportional", "gain", Screen::Settings, 1, 3); break;
    case 7: openDigitEditor("Front I", "x", &workingSettings.frontPidI_x1000, 0, 9999, "Front-hold integral", "gain", Screen::Settings, 1, 3); break;
    case 8: openDigitEditor("Front D", "x", &workingSettings.frontPidD_x1000, 0, 9999, "Front-hold derivative", "gain", Screen::Settings, 1, 3); break;
    case 9: openDigitEditor("Turn yaw", "deg", &workingSettings.turnYawTolerance_x100, 0, 9999, "Yaw window before", "heading swap", Screen::Settings, 2, 2); break;
    case 10: openDigitEditor("Wall thr", "cm", &workingSettings.corridorWallThreshold_x100, 0, 9999, "Wall limit for PID", "and finish detection", Screen::Settings, 2, 2); break;
    case 11: openDigitEditor("Turn detect", "cm", &workingSettings.turnDetectDistance_x100, 0, 9999, "Upper front range", "that still means turn", Screen::Settings, 2, 2); break;
    case 12: openDigitEditor("Dead-end", "cm", &workingSettings.deadEndDistance_x100, 0, 9999, "Front distance limit", "for dead-end event", Screen::Settings, 2, 2); break;
    case 13: openDigitEditor("Finish", "cm", &workingSettings.finishDistance_x100, 0, 9999, "Front distance needed", "before finish counts", Screen::Settings, 2, 2); break;
    case 14: openDigitEditor("Check gap", "s", &workingSettings.sensingInterval_x100, 1, 9999, "Time between event", "verification checks", Screen::Settings, 2, 2); break;
    case 15: openDigitEditor("US avg k", "x", &workingSettings.ultrasonicAvgCount, 1, ULTRASONIC_AVG_COUNT_MAX, "How many ultrasonic", "samples are averaged", Screen::Settings, 1, 0); break;
    case 16: openDigitEditor("Match N", "x", &workingSettings.eventConfirmCount, 1, 9, "How many identical", "events confirm state", Screen::Settings, 1, 0); break;
    case 17: gCurrentScreen = Screen::Root; break;
    default: break;
  }
}
...
void updateEventTestRun() {
  ...
  switch (gEventTestRun.state) {
    ...
    case NavState::FrontNormalize:
      ...
      runFrontNormalization(gEventTestRun.heading, frame);
      if (frontNormalizationComplete(frame)) {
        gEventTestRun.heading = gEventTestRun.pendingHeading;
        resetDrivePidControl();
...
void drawSettingsScreen() {
  ...
  snprintf(items[5], sizeof(items[5]), "FrontRef:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidP_x1000, 3);
  snprintf(items[6], sizeof(items[6]), "FrontP:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidI_x1000, 3);
  snprintf(items[7], sizeof(items[7]), "FrontI:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.frontPidD_x1000, 3);
  snprintf(items[8], sizeof(items[8]), "FrontD:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.turnYawTolerance_x100, 2);
  snprintf(items[9], sizeof(items[9]), "YawTol:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.corridorWallThreshold_x100, 2);
  snprintf(items[10], sizeof(items[10]), "WallThr:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.turnDetectDistance_x100, 2);
  snprintf(items[11], sizeof(items[11]), "TurnDet:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.deadEndDistance_x100, 2);
  snprintf(items[12], sizeof(items[12]), "DeadEnd:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.finishDistance_x100, 2);
  snprintf(items[13], sizeof(items[13]), "Finish:%s", value);
  formatCompactValue(value, sizeof(value), activeSettings.sensingInterval_x100, 2);
  snprintf(items[14], sizeof(items[14]), "CheckGap:%s", value);
  snprintf(items[15], sizeof(items[15]), "USavg:%d", activeSettings.ultrasonicAvgCount);
  snprintf(items[16], sizeof(items[16]), "Match N:%d", activeSettings.eventConfirmCount);
  snprintf(items[17], sizeof(items[17]), "Back");
  const char* ptrs[18];
  for (int i = 0; i < 18; ++i) ptrs[i] = items[i];
  drawScrollableItemList(ptrs, 18, gSettingsIndex, 26);
  u8g2.setFont(u8g2_font_5x8_tf);
  u8g2.drawStr(0, 63, "Long: root");
}
