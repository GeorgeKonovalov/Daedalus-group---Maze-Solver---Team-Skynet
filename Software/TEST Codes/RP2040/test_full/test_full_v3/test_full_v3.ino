...
constexpr int EVENT_TEST_COUNT = 9;
...
void handleEventTestMenuInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    ...
    currentEventName = eventTestCaseToEventName(selectedEventTest);
    currentHeading = Heading::North;
...
void drawEventTestRunScreen() {
  ...
  String line6 = String("IR F:") + String(irFL) + "/" + String(irFR);
  String line7 = String("IR R:") + String(irRL) + "/" + String(irRR);
...
void drawIRSensorTestScreen() {
  ...
  String line1 = String("FL: ") + String(irFL) + "  FR: " + String(irFR);
  String line2 = String("RL: ") + String(irRL) + "  RR: " + String(irRR);
  u8g2.drawStr(0, 28, line1.c_str());
  u8g2.drawStr(0, 42, line2.c_str());
  u8g2.setFont(u8g2_font_5x8_tf);
  u8g2.drawStr(0, 56, "Front: FL FR");
...
void drawRunScreen() {
  ...
  String line6 = "IR FL:" + String(irFL) + " FR:" + String(irFR);
  String line7 = "IR RL:" + String(irRL) + " RR:" + String(irRR);
...
void runSelectedEventTest() {
  ...
  switch (selectedEventTest) {
    ...
    case EventTestCase::TJunction:
      currentEventName = EventName::TJunction;
      if (ultrasonicFresh() && frontDistance() > gJunctionStopDistance) {
    ...
    case EventTestCase::DeadEnd:
      ...
      if (ultrasonicFresh() && frontDistance() > FRONT_STOP_THRESHOLD) {
        driveHeading(APPROACH_SPEED, 0.0f);
      } else {
        stopAll();
      }
    ...
    case EventTestCase::LeftTurn:
      ...
      currentEventName = EventName::LeftTurn;
      driveHeading(BASE_SPEED, 0.0f);
      break;

    case EventTestCase::RightTurn:
      currentHeading = Heading::East;
      currentEventName = EventName::RightTurn;
      driveHeading(BASE_SPEED, 0.0f);
      break;
