...
const char* stateName(RobotState s) {
    switch (s) {
        ...
        case ST_ENTERING:       return "ENTERING";
...
/*
  Type single characters in the serial monitor to test the display:
    ...
    0              — state: ST_STARTUP
    1              — state: ST_WAIT_FOR_ENTRY
    2              — state: ST_ENTERING
    3              — state: ST_FOLLOWING
    4              — state: ST_SCAN_EVENT
    5              — state: ST_DECIDE
    6              — state: ST_COMPLETE
    7              — state: ST_COOLDOWN
...
void checkSerialCommands() {
    ...
    switch (c) {
        ...
        case '1': state = ST_WAIT_FOR_ENTRY; Serial.println(">> State: WAITING");    break;
        case '2': state = ST_ENTERING;       Serial.println(">> State: ENTERING");   break;
        case '3': state = ST_FOLLOWING;      Serial.println(">> State: FOLLOWING");  break;
        case '4': state = ST_SCAN_EVENT;     Serial.println(">> State: SCAN_EVENT"); break;
        case '5': state = ST_DECIDE;         Serial.println(">> State: DECIDE");     break;
        case '6': state = ST_COMPLETE;       Serial.println(">> State: COMPLETE");   break;
        case '7': state = ST_COOLDOWN;       Serial.println(">> State: COOLDOWN");   break;
