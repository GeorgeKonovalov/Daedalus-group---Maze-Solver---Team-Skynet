...
int rootIndex = 0;          // 0=Start, 1=Start Settings, 2=Settings, 3=Event Test, 4=IR, 5=US
...
bool eventDetectedForTest(EventTestCase testCase);
...
bool eventDetectedForTest(EventTestCase testCase) {
  ...
  switch (testCase) {
    case EventTestCase::Corridor:
      return true;
    case EventTestCase::DeadEnd:
      return ultrasonicFresh() && frontDistance() <= stopThreshold && !leftIsOpen && !rightIsOpen;
    case EventTestCase::LeftTurn:
      return leftIsOpen && !rightIsOpen;
    case EventTestCase::RightTurn:
      return rightIsOpen && !leftIsOpen;
    case EventTestCase::TJunction:
    case EventTestCase::TJunctionStraight:
      return leftIsOpen && rightIsOpen;
    case EventTestCase::Start:
      return frontIRTriggered() && sideUltrasonicWithin(currentEventTestSideThreshold());
    case EventTestCase::Finish:
      return finishIRTriggered();
    case EventTestCase::Random:
      return leftIsOpen || rightIsOpen || finishIRTriggered() ||
             (ultrasonicFresh() && frontDistance() <= stopThreshold);
...
void handleRootInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (rootIndex) {
      ...
      case 4: irSensorIndex = 0; currentScreen = Screen::IRSensorTest; break;
      case 5: currentScreen = Screen::UltrasonicSensorTest; break;
    }
  }
}
...
void drawUltrasonicSensorTestScreen() {
  ...
  String line1 = String("N: ") + formatDistanceCm(distN) + "  E: " + formatDistanceCm(distE);
  String line2 = String("S: ") + formatDistanceCm(distS) + "  W: " + formatDistanceCm(distW);
  u8g2.drawStr(0, 26, line1.c_str());
