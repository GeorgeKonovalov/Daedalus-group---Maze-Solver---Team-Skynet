...
SensorObservation buildSensorObservation(const SensorSnapshot& snapshot, Heading heading, float deadEndThresholdCm,
                                        float turningThresholdCm, float wallThresholdCm);
...
SensorObservation buildSensorObservation(const SensorSnapshot& snapshot, Heading heading, float deadEndThresholdCm,
  ...
  if (front <= deadEndThresholdCm) {
    observation.frontZone = 0;
  } else if (front < turningThresholdCm) {
    observation.frontZone = 1;
  } else if (front < (deadEndThresholdCm * 2.0f)) {
    observation.frontZone = 2;
  } else {
    observation.frontZone = 3;
  }

  observation.leftZone = (left <= wallThresholdCm) ? 0 : 1;
  observation.rightZone = (right <= wallThresholdCm) ? 0 : 1;
  return observation;
}

bool sensorObservationChanged(const SensorObservation& a, const SensorObservation& b) {
  return a.irMask != b.irMask ||
         a.frontZone != b.frontZone ||
         a.leftZone != b.leftZone ||
         a.rightZone != b.rightZone ||
         a.ultrasonicValid != b.ultrasonicValid;
}

bool sensorSnapshotChanged(const SensorSnapshot& a, const SensorSnapshot& b) {
  SensorObservation obsA = buildSensorObservation(a, currentHeading, gDeadEndDistanceCm, gTurningThresholdCm, gWallThresholdCm);
  SensorObservation obsB = buildSensorObservation(b, currentHeading, gDeadEndDistanceCm, gTurningThresholdCm, gWallThresholdCm);
...
EventName classifySensorEventWithThresholds(const SensorSnapshot& snapshot, Heading heading, bool allowFinishEvent,
  ...
  bool frontPairTriggered = frontPairTriggeredMask(snapshot.irMask);
  bool rearPairTriggered = rearPairTriggeredMask(snapshot.irMask);
  bool irChangedFromCorridor = !allTriggered && !allClear;

  float front = distanceForHeadingInSnapshot(snapshot, heading);
  float left = distanceForHeadingInSnapshot(snapshot, turnLeftOf(heading));
  float right = distanceForHeadingInSnapshot(snapshot, turnRightOf(heading));
  bool leftOpenStable = left > wallThresholdCm;
  bool rightOpenStable = right > wallThresholdCm;
  ...
  if (irChangedFromCorridor && front > deadEndThresholdCm && front < turningThresholdCm) {
  ...
  if (irChangedFromCorridor && front >= turningThresholdCm &&
      leftOpenStable && rightOpenStable) {
...
EventName classifyStableSensorEvent(const SensorSnapshot& snapshot, Heading heading, bool allowFinishEvent) {
  return classifySensorEventWithThresholds(snapshot, heading, allowFinishEvent,
                                          gDeadEndDistanceCm, gTurningThresholdCm, gWallThresholdCm);
}
...
float frontDistanceForHeading(Heading h) { return distanceForHeading(h); }
float leftDistanceForHeading(Heading h) { return distanceForHeading(turnLeftOf(h)); }
float rightDistanceForHeading(Heading h) { return distanceForHeading(turnRightOf(h)); }
...
bool leftOpen() {
  return ultrasonicFresh() && leftDistance() > gWallThresholdCm;
}

bool rightOpen() {
  return ultrasonicFresh() && rightDistance() > gWallThresholdCm;
}
...
bool hasLeftWallWithin(float threshold) {
  return ultrasonicFresh() && leftDistance() <= threshold;
}

bool hasRightWallWithin(float threshold) {
  return ultrasonicFresh() && rightDistance() <= threshold;
}
...
Heading chooseNextHeadingFromSnapshot(const SensorSnapshot& snapshot, Heading heading) {
  ...
  bool leftIsOpen = left > gWallThresholdCm;
  bool rightIsOpen = right > gWallThresholdCm;
...
void updateMazeTestLogic() {
  ...
  switch (mazeTestPhase) {
    ...
    case MazeTestPhase::StartSeek:
      mazeTestEvent = EventName::Start;
      mazeTestResolvedEvent = EventName::Start;
      if (hasStableEvent && stableDetectedEvent == EventName::Corridor) {
    ...
    case MazeTestPhase::ApproachWall: {
      ...
      bool leftIsOpen = distanceForHeadingInSnapshot(stableEventSnapshot, turnLeftOf(mazeTestHeading)) > gWallThresholdCm;
      bool rightIsOpen = distanceForHeadingInSnapshot(stableEventSnapshot, turnRightOf(mazeTestHeading)) > gWallThresholdCm;
      ...
      switch (gMode) {
        ...
        case 4:
          if (leftIsOpen && rightIsOpen) {
            mazeTestPendingHeading =
              (distanceForHeadingInSnapshot(stableEventSnapshot, turnLeftOf(mazeTestHeading)) >=
               distanceForHeadingInSnapshot(stableEventSnapshot, turnRightOf(mazeTestHeading)))
              ? leftHeading
              : rightHeading;
          } else if (leftIsOpen) {
    ...
    case MazeTestPhase::AcquireWall:
      mazeTestEvent = mazeTestResolvedEvent;
      if (ultrasonicFresh() &&
          leftDistanceForHeading(mazeTestHeading) <= gWallThresholdCm &&
          rightDistanceForHeading(mazeTestHeading) <= gWallThresholdCm) {
        mazeTestPhase = MazeTestPhase::Corridor;
      }
      return;
...
void runSelectedEventTest() {
  ...
  auto resolveRandomEvent = [&]() -> EventName {
    return classifySensorEventWithThresholds(stableEventSnapshot, currentHeading, true,
                                            gDeadEndDistanceCm, gTurningThresholdCm, sideThreshold);
  };
  ...
  switch (eventTestPhase) {
    ...
    case EventTestPhase::StartSeek:
      ...
      if (hasStableEvent &&
          classifySensorEventWithThresholds(stableEventSnapshot, currentHeading, false,
                                            gDeadEndDistanceCm, gTurningThresholdCm, sideThreshold) == EventName::Corridor) {
        eventTestPhase = EventTestPhase::Detect;
        eventTestStartedMs = millis();
        previousError = 0.0f;
        integral = 0.0f;
    ...
    case EventTestPhase::Detect: {
      ...
      EventName stableDetectedEvent = classifySensorEventWithThresholds(stableEventSnapshot, currentHeading, true,
        gDeadEndDistanceCm, gTurningThresholdCm, sideThreshold);
...
void updateExecutionLogic(bool& stopNow, EventName& displayedEvent) {
  ...
  switch (executionPhase) {
    ...
    case ExecutionPhase::Corridor:
      ...
      if (hasStableEvent && stableDetectedEvent == EventName::TJunctionStraight) {
