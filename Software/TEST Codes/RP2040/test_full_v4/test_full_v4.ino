...
EventTestConfig eventTestConfigs[9];
...
void handleIRSensorTestInput(ButtonEvent btn);
...
EventTestConfig& currentEventTestConfig() {
  return eventTestConfigs[static_cast<int>(selectedEventTest)];
}
...
void handleRootInput(int delta, ButtonEvent btn) {
  ...
  if (btn == ButtonEvent::ShortPress) {
    switch (rootIndex) {
      ...
      case 4: currentScreen = Screen::IRSensorTest; break;
...
void drawIRSensorTestScreen() {
  ...
  String line1 = String("NW: ") + String(irRL) + " NE: " + String(irRR);
  String line2 = String("SW: ") + String(irFL) + " SE: " + String(irFR);
  u8g2.drawStr(0, 28, line1.c_str());
  u8g2.drawStr(0, 42, line2.c_str());
  u8g2.setFont(u8g2_font_5x8_tf);
  u8g2.drawStr(0, 56, "N=RL/RR S=FL/FR");
...
void runMenuInSetup() {
  ...
  while (!programStarted) {
    ...
    switch (currentScreen) {
      ...
      case Screen::IRSensorTest:  handleIRSensorTestInput(btn); break;
...
void runSelectedEventTest() {
  ...
  auto resolveRandomEvent = [&]() -> EventName {
    if (finishIRTriggered()) return EventName::Finish;
    if (leftIsOpen && rightIsOpen) return EventName::TJunction;
    if (leftIsOpen) return EventName::LeftTurn;
    if (rightIsOpen) return EventName::RightTurn;
    if (ultrasonicFresh() && frontDistance() <= stopThreshold) return EventName::DeadEnd;
    return EventName::Corridor;
  };
  ...
  switch (eventTestPhase) {
    ...
    case EventTestPhase::StartSeek:
      currentEventName = EventName::Start;
      if (frontIRTriggered() && sideUltrasonicWithin(sideThreshold)) {
        eventTestPhase = EventTestPhase::Detect;
        eventTestStartedMs = millis();
        previousError = 0.0f;
        integral = 0.0f;
    ...
    case EventTestPhase::Detect:
      ...
      if (!eventDetectedForTest(selectedEventTest)) {
        currentEventName = EventName::Corridor;
        followCorridor();
        return;
      }
...
void updateExecutionLogic(bool& stopNow, EventName& displayedEvent) {
  ...
  switch (executionPhase) {
    case ExecutionPhase::Corridor:
      if (leftIsOpen && rightIsOpen && front > gJunctionStopDistance) {
        executionPhase = ExecutionPhase::ApproachWall;
        phaseStartedMs = millis();
        displayedEvent = EventName::TJunction;
        driveHeading(APPROACH_SPEED, 0.0f);
        return;
      }
